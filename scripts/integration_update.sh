...
#
# `vdb start`'s new-release notice and `vdb update`, end to end, in the Linux dev
...
PORT=18931
export VDB_UPDATE_BASE_URL="http://127.0.0.1:$PORT"
V="$T/bin/vdb"
ENGINE="vdb-linux-$ARCH"
PASS=0
FAIL=0
HTTP_PID=""
...
ledger_digest() { pg "$1" "SELECT count(*) || ':' || coalesce(md5(string_agg(row_hash, ',' ORDER BY id)), '') FROM vdb.schema_ledger WHERE id <= ${2:-0}"; }
EMAIL="updtest@vectoradb.dev"
...
build() { (cd "$ROOT" && go build -ldflags "-X github.com/vectoradb/vectoradb/internal/version.Version=$1" -o "$2" ./cmd/vdb); }
...
python3 - "$VDB_UPDATE_BASE_URL" "$T/www" "$ENGINE" <<'PY'
...
/usr/local/bin/vdb stop >/dev/null 2>&1
"$V" stop >/dev/null 2>&1; sleep 1
VECTORADB_NO_UPDATE_CHECK=1 "$V" start >/dev/null 2>&1; sleep 5
pg vec-main "SET vdb.allow_destructive=on; DROP TABLE IF EXISTS updkeep; CREATE TABLE updkeep(id int); INSERT INTO updkeep SELECT generate_series(1,3);" >/dev/null
"$V" branch delete updb >/dev/null 2>&1
"$V" branch create updb >/dev/null 2>&1
pg vec-updb "CREATE TABLE onbranch(x int); INSERT INTO onbranch VALUES (7);" >/dev/null
MAIN_ID="$(sudo docker inspect -f '{{.Id}}' vec-main)"
BR_ID="$(sudo docker inspect -f '{{.Id}}' vec-updb)"
SECRETS="$(md5sum "$HOME/.vectoradb/secrets.json" 2>/dev/null)"
TLS="$(cd "$HOME/.vectoradb" && find tls -type f -exec md5sum {} + 2>/dev/null | sort)"
MAIN_MAX="$(pg vec-main "SELECT max(id) FROM vdb.schema_ledger")"
BR_MAX="$(pg vec-updb "SELECT max(id) FROM vdb.schema_ledger")"
MAIN_HIST="$(ledger_digest vec-main "$MAIN_MAX")"
BR_HIST="$(ledger_digest vec-updb "$BR_MAX")"
printf 'password123\n' | "$V" user create "$EMAIL" >/dev/null 2>&1 || true
KEY="$("$V" apikey create "$EMAIL" upd 2>/dev/null | grep -o 'vdb_[A-Za-z0-9_-]*')"
...
assert_eq "branch has its data" "$(pg vec-updb 'SELECT x FROM onbranch')" "7"
assert_eq "control API runs on 0.98.0" "$(readlink "/proc/$CP_PID/exe")" "$V"

echo "### 1. vdb start prints the notice, last"
OUT="$("$V" start 2>&1)"
...
echo "### 4-5. vdb update --check picks the newest complete, non-prerelease release"
OUT="$("$V" update --check 2>&1)"; CODE=$?
assert_eq "--check exits 0" "$CODE" "0"
contains "--check reports v0.99.0" "VectoraDB v0.99.0 is available (you have 0.98.0)." "$OUT"
lacks "prerelease v1.0.0 not offered" "v1.0.0" "$OUT"
lacks "incomplete v0.99.5 not offered" "v0.99.5" "$OUT"
assert_eq "--check changes nothing" "$("$V" version)" "vdb 0.98.0"
...
assert_eq "still 0.98.0" "$("$V" version)" "vdb 0.98.0"
assert_eq "servers untouched" "$(pid_of 'control API')" "$CP_PID"
assert_eq "no partial download left" "$(ls "$HOME/.vectoradb/updates/v0.99.0" 2>/dev/null | wc -l | tr -d ' ')" "0"
cp "$T/good-engine" "$T/www/dl/v0.99.0/$ENGINE"

echo "### 7. vdb update --yes"
OUT="$("$V" update --yes 2>&1)"; CODE=$?
assert_eq "update exits 0" "$CODE" "0"
[ "$CODE" = 0 ] || tail -n 30 <<<"$OUT" | sed 's/^/      /'
contains "says done" "Done — VectoraDB is now v0.99.0. Your data was not touched." "$OUT"
contains "upgraded Blackbox on the branch" "updb: ledger up to date" "$OUT"
assert_eq "vdb is 0.99.0" "$("$V" version)" "vdb 0.99.0"
...
assert_eq "main container not recreated" "$(sudo docker inspect -f '{{.Id}}' vec-main)" "$MAIN_ID"
assert_eq "branch container not recreated" "$(sudo docker inspect -f '{{.Id}}' vec-updb)" "$BR_ID"
assert_eq "main data intact" "$(pg vec-main 'SELECT count(*) FROM updkeep')" "3"
assert_eq "branch data intact" "$(pg vec-updb 'SELECT x FROM onbranch')" "7"
# Starting and upgrading add entries; every entry from before must be untouched.
assert_eq "main Blackbox history unchanged" "$(ledger_digest vec-main "$MAIN_MAX")" "$MAIN_HIST"
assert_eq "branch Blackbox history unchanged" "$(ledger_digest vec-updb "$BR_MAX")" "$BR_HIST"
...
assert_eq "blast radius present on the branch" "$(pg vec-updb "SELECT count(*) > 0 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace WHERE n.nspname = 'vdb' AND p.proname = 'blast_radius'")" "t"
assert_eq "secrets.json unchanged" "$(md5sum "$HOME/.vectoradb/secrets.json" 2>/dev/null)" "$SECRETS"
assert_eq "TLS certificates unchanged" "$(cd "$HOME/.vectoradb" && find tls -type f -exec md5sum {} + 2>/dev/null | sort)" "$TLS"
assert_eq "previous engine kept" "$("$HOME/.vectoradb/updates/prev/vdb" version)" "vdb 0.98.0"
assert_eq "gateway serves main with a key made before the update" "$(PGPASSWORD="$KEY" psql "postgresql://vectoradb@127.0.0.1:6432/main?sslmode=require" -tAc 'SELECT count(*) FROM updkeep' 2>&1)" "3"
