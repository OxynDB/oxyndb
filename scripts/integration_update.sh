...
#
# `odb start`'s new-release notice and `odb update`, end to end, in the Linux dev
...
# stack is put back on /usr/local/bin/odb at the end.
set -uo pipefail
...
PORT=18931
export ODB_UPDATE_BASE_URL="http://127.0.0.1:$PORT"
V="$T/bin/odb"
ENGINE="odb-linux-$ARCH"
PASS=0
FAIL=0
HTTP_PID=""
...
ledger_digest() { pg "$1" "SELECT count(*) || ':' || coalesce(md5(string_agg(row_hash, ',' ORDER BY id)), '') FROM odb.schema_ledger WHERE id <= ${2:-0}"; }
EMAIL="updtest@oxyndb.dev"
...
cleanup() {
	...
	pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS updkeep" >/dev/null
	"$V" branch delete updb >/dev/null 2>&1
	rm -rf "$HOME/.oxyndb/updates" "$HOME/.oxyndb/update-check.json" "$HOME/.oxyndb/update-notice.json"
	# Put the stack back on the installed binary.
	"$V" stop >/dev/null 2>&1
	/usr/local/bin/odb start >/dev/null 2>&1
	rm -rf "$T"
}
trap cleanup EXIT

echo "### setup: build test releases 0.98.0 and 0.99.0, serve a fake GitHub"
build() { (cd "$ROOT" && go build -ldflags "-X github.com/OxynDB/oxyndb/internal/version.Version=$1" -o "$2" ./cmd/odb); }
...
python3 - "$ODB_UPDATE_BASE_URL" "$T/www" "$ENGINE" <<'PY'
...
os.makedirs(os.path.join(www, "repos/OxynDB/oxyndb"), exist_ok=True)
json.dump(rels, open(os.path.join(www, "repos/OxynDB/oxyndb/releases"), "w"))
PY
start_fake_github() {
	python3 -m http.server "$PORT" --bind 127.0.0.1 --directory "$T/www" >/dev/null 2>&1 &
	HTTP_PID=$!
	for _ in $(seq 50); do curl -sf "$ODB_UPDATE_BASE_URL/repos/OxynDB/oxyndb/releases" >/dev/null && break; sleep 0.2; done
}
stop_fake_github() { [ -n "$HTTP_PID" ] && kill "$HTTP_PID" 2>/dev/null; HTTP_PID=""; }
start_fake_github
assert_eq "test binary is 0.98.0" "$("$V" version)" "odb 0.98.0"

# Stack on the 0.98.0 binary, with data on main and on a branch.
/usr/local/bin/odb stop >/dev/null 2>&1
"$V" stop >/dev/null 2>&1; sleep 1
OXYNDB_NO_UPDATE_CHECK=1 "$V" start >/dev/null 2>&1; sleep 5
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS updkeep; CREATE TABLE updkeep(id int); INSERT INTO updkeep SELECT generate_series(1,3);" >/dev/null
"$V" branch delete updb >/dev/null 2>&1
"$V" branch create updb >/dev/null 2>&1
pg oxyn-updb "CREATE TABLE onbranch(x int); INSERT INTO onbranch VALUES (7);" >/dev/null
MAIN_ID="$(sudo docker inspect -f '{{.Id}}' oxyn-main)"
BR_ID="$(sudo docker inspect -f '{{.Id}}' oxyn-updb)"
SECRETS="$(md5sum "$HOME/.oxyndb/secrets.json" 2>/dev/null)"
TLS="$(cd "$HOME/.oxyndb" && find tls -type f -exec md5sum {} + 2>/dev/null | sort)"
MAIN_MAX="$(pg oxyn-main "SELECT max(id) FROM odb.schema_ledger")"
BR_MAX="$(pg oxyn-updb "SELECT max(id) FROM odb.schema_ledger")"
MAIN_HIST="$(ledger_digest oxyn-main "$MAIN_MAX")"
BR_HIST="$(ledger_digest oxyn-updb "$BR_MAX")"
printf 'password123\n' | "$V" user create "$EMAIL" >/dev/null 2>&1 || true
KEY="$("$V" apikey create "$EMAIL" upd 2>/dev/null | grep -o 'odb_[A-Za-z0-9_-]*')"
...
assert_eq "branch has its data" "$(pg oxyn-updb 'SELECT x FROM onbranch')" "7"
assert_eq "control API runs on 0.98.0" "$(readlink "/proc/$CP_PID/exe")" "$V"

echo "### 1. odb start prints the notice, last"
OUT="$("$V" start 2>&1)"
assert_eq "notice is the last line" "$(tail -n 1 <<<"$OUT")" \
	'OxynDB v0.99.0 is available (you have 0.98.0). Run `odb update` to get the new capabilities.'
assert_eq "notice printed once" "$(grep -c 'is available' <<<"$OUT")" "1"
contains "banner still printed" "OxynDB is up (background)" "$OUT"
...
stop_fake_github
lacks "a forced check with GitHub unreachable prints nothing" "is available" "$(OXYNDB_UPDATE_CHECK_INTERVAL=0 "$V" start 2>&1)"
assert_eq "the remembered notice still prints with GitHub unreachable" "$(tail -n 1 <<<"$("$V" start 2>&1)")" \
	'OxynDB v0.99.0 is available (you have 0.98.0). Run `odb update` to get the new capabilities.'
start_fake_github

echo "### 2. OXYNDB_NO_UPDATE_CHECK=1 turns it off"
lacks "no notice when turned off" "is available" "$(OXYNDB_NO_UPDATE_CHECK=1 OXYNDB_UPDATE_CHECK_INTERVAL=0 "$V" start 2>&1)"

echo "### 3. offline: nothing printed, no noticeable delay"
S=$(date +%s)
OUT="$(ODB_UPDATE_BASE_URL=http://127.0.0.1:9 OXYNDB_UPDATE_CHECK_INTERVAL=0 "$V" start 2>&1)"
...
echo "### 4-5. odb update --check picks the newest complete, non-prerelease release"
OUT="$("$V" update --check 2>&1)"; CODE=$?
assert_eq "--check exits 0" "$CODE" "0"
contains "--check reports v0.99.0" "OxynDB v0.99.0 is available (you have 0.98.0)." "$OUT"
lacks "prerelease v1.0.0 not offered" "v1.0.0" "$OUT"
lacks "incomplete v0.99.5 not offered" "v0.99.5" "$OUT"
assert_eq "--check changes nothing" "$("$V" version)" "odb 0.98.0"
...
assert_eq "still 0.98.0" "$("$V" version)" "odb 0.98.0"
assert_eq "servers untouched" "$(pid_of 'control API')" "$CP_PID"
assert_eq "no partial download left" "$(ls "$HOME/.oxyndb/updates/v0.99.0" 2>/dev/null | wc -l | tr -d ' ')" "0"
cp "$T/good-engine" "$T/www/dl/v0.99.0/$ENGINE"

echo "### 7. odb update --yes"
OUT="$("$V" update --yes 2>&1)"; CODE=$?
assert_eq "update exits 0" "$CODE" "0"
[ "$CODE" = 0 ] || tail -n 30 <<<"$OUT" | sed 's/^/      /'
contains "says done" "Done — OxynDB is now v0.99.0. Your data was not touched." "$OUT"
contains "upgraded Blackbox on the branch" "updb: ledger up to date" "$OUT"
assert_eq "odb is 0.99.0" "$("$V" version)" "odb 0.99.0"
...
assert_eq "main container not recreated" "$(sudo docker inspect -f '{{.Id}}' oxyn-main)" "$MAIN_ID"
assert_eq "branch container not recreated" "$(sudo docker inspect -f '{{.Id}}' oxyn-updb)" "$BR_ID"
assert_eq "main data intact" "$(pg oxyn-main 'SELECT count(*) FROM updkeep')" "3"
assert_eq "branch data intact" "$(pg oxyn-updb 'SELECT x FROM onbranch')" "7"
# Starting and upgrading add entries; every entry from before must be untouched.
assert_eq "main Blackbox history unchanged" "$(ledger_digest oxyn-main "$MAIN_MAX")" "$MAIN_HIST"
assert_eq "branch Blackbox history unchanged" "$(ledger_digest oxyn-updb "$BR_MAX")" "$BR_HIST"
...
assert_eq "blast radius present on the branch" "$(pg oxyn-updb "SELECT count(*) > 0 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace WHERE n.nspname = 'odb' AND p.proname = 'blast_radius'")" "t"
assert_eq "secrets.json unchanged" "$(md5sum "$HOME/.oxyndb/secrets.json" 2>/dev/null)" "$SECRETS"
assert_eq "TLS certificates unchanged" "$(cd "$HOME/.oxyndb" && find tls -type f -exec md5sum {} + 2>/dev/null | sort)" "$TLS"
assert_eq "previous engine kept" "$("$HOME/.oxyndb/updates/prev/odb" version)" "odb 0.98.0"
assert_eq "gateway serves main with a key made before the update" "$(PGPASSWORD="$KEY" psql "postgresql://oxyndb@127.0.0.1:6432/main?sslmode=require" -tAc 'SELECT count(*) FROM updkeep' 2>&1)" "3"
...
contains "--check says up to date" "OxynDB 0.99.0 is up to date." "$("$V" update --check 2>&1)"
lacks "start shows no notice" "is available" "$(OXYNDB_UPDATE_CHECK_INTERVAL=0 "$V" start 2>&1)"
