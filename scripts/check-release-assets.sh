...
#
# Fails unless a release has every file `vdb update` needs, attached AND listed
...
required=(
	...
	vdb-linux-arm64
	vdb-linux-amd64
	vdb-windows-amd64.exe
	vectoradb-docker-context.tar.gz
)
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
...
check() {
	...
	for name in "${required[@]}"; do
		if ! grep -qxF "$name" <<<"$attached"; then
			echo "::error::$tag is missing $name (vdb update needs it)"
			missing=1
		fi
		if ! awk -v n="$name" '{ f = $2; sub(/^\*/, "", f); sub(/\r$/, "", f) } f == n { found = 1 } END { exit !found }' "$tmp/SHA256SUMS"; then
			echo "::error::SHA256SUMS of $tag doesn't list $name (vdb update needs it)"
			missing=1
		fi
	done
	return "$missing"
...
for attempt in 1 2 3; do
	if check; then
		echo "$tag has every file vdb update needs"
		exit 0
	fi
