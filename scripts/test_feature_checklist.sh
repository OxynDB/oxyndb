...
#
# Keeps docs/ODB_Checklist.html in step with docs/ODB_Feature_Implemented.html:
...
feature="$docs/ODB_Feature_Implemented.html"
checklist="$docs/ODB_Checklist.html"
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
FAIL=0
...
[ -z "$missing" ] || { echo "FAIL: in ODB_Feature_Implemented but not in ODB_Checklist: $(echo $missing)"; FAIL=1; }
[ -z "$extra" ]   || { echo "FAIL: in ODB_Checklist but not in ODB_Feature_Implemented: $(echo $extra)"; FAIL=1; }
[ -z "$dups" ]    || { echo "FAIL: listed twice in ODB_Checklist: $(echo $dups)"; FAIL=1; }

bad_st="$(grep -o 'data-st="[^"]*"' "$checklist" | grep -v 'data-st="\(done\|man\|part\|bad\|todo\)"' | sort -u)"
[ -z "$bad_st" ] || { echo "FAIL: unknown status in ODB_Checklist: $bad_st"; FAIL=1; }
...
[ -z "$mismatch" ] || { echo "FAIL: status and badge disagree in ODB_Checklist: $mismatch"; FAIL=1; }

if [ "$FAIL" = 0 ]; then
	echo "feature checklist: $(wc -l < "$tmp/checklist" | tr -d ' ') features in step with ODB_Feature_Implemented"
fi
exit "$FAIL"
