...
#
# VDB_TEST_ALLOW_REAL_INSTALL=yes-destroy-my-data overrides this, for a
# machine that is itself disposable (for example a CI runner).
if [ ! -f /etc/vdb-test-instance ] && [ "${VDB_TEST_ALLOW_REAL_INSTALL:-}" != "yes-destroy-my-data" ]; then
	cat >&2 <<'MSG'
refusing to run: this is not a VectoraDB test instance.
...
VDB_TEST_ALLOW_REAL_INSTALL=yes-destroy-my-data.
MSG
	exit 2
fi
