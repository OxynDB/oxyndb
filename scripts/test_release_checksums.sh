...
for f in vdb-linux-amd64 vdb-verify-linux-amd64 vectoradb-docker-context.tar.gz; do echo "content of $f" > "$tmp/rel/$f"; done
echo "content of the distro image" > "$tmp/distro/vectoradb-distro.tar.gz"

publish_release() { (cd "$tmp/rel" && bash "$script" v9.9.9 vdb-linux-amd64 vdb-verify-linux-amd64 vectoradb-docker-context.tar.gz) >/dev/null; }
publish_distro()  { (cd "$tmp/distro" && bash "$script" v9.9.9 vectoradb-distro.tar.gz) >/dev/null; }
...
assert_eq "a release without SHA256SUMS gets this job's entries" "$(names)" \
  "vdb-linux-amd64 vdb-verify-linux-amd64 vectoradb-docker-context.tar.gz"

publish_distro
assert_eq "the second job adds its entry and keeps the first job's" "$(names)" \
  "vdb-linux-amd64 vdb-verify-linux-amd64 vectoradb-distro.tar.gz vectoradb-docker-context.tar.gz"
...
echo "changed binary" > "$tmp/rel/vdb-linux-amd64"
publish_release
assert_eq "a re-run replaces its own entry without duplicating it" \
  "$(grep -c ' vdb-linux-amd64$' "$STUB_RELEASE/SHA256SUMS")|$(grep ' vdb-linux-amd64$' "$STUB_RELEASE/SHA256SUMS" | awk '{print $1}')|$(wc -l < "$STUB_RELEASE/SHA256SUMS" | tr -d ' ')" \
  "1|$(sum "$tmp/rel/vdb-linux-amd64")|4"
assert_eq "…and leaves the other job's entry untouched" \
  "$(grep ' vectoradb-distro.tar.gz$' "$STUB_RELEASE/SHA256SUMS" | awk '{print $1}')" "$(sum "$tmp/distro/vectoradb-distro.tar.gz")"

new_release exact
printf '%s  %s\n' "$(printf 'a%.0s' $(seq 1 64))" "vdb-linux-amd64.sig" > "$STUB_RELEASE/SHA256SUMS"
publish_release
assert_eq "entries are matched by exact name (vdb-linux-amd64.sig survives)" "$(names)" \
  "vdb-linux-amd64 vdb-linux-amd64.sig vdb-verify-linux-amd64 vectoradb-docker-context.tar.gz"

new_release race
printf '%s  %s\n' "$(printf 'b%.0s' $(seq 1 64))" "vectoradb-distro.tar.gz" > "$tmp/other-job.txt"
export STUB_OVERWRITE="$tmp/other-job.txt"
out="$( (cd "$tmp/rel" && bash "$script" v9.9.9 vdb-linux-amd64 vdb-verify-linux-amd64 vectoradb-docker-context.tar.gz) 2>&1 >/dev/null)"
assert_eq "another job uploading at the same moment is detected and merged again" \
  "$(echo "$out" | grep -c 'merging again')|$(names)" \
  "1|vdb-linux-amd64 vdb-verify-linux-amd64 vectoradb-distro.tar.gz vectoradb-docker-context.tar.gz"

new_release failing
export STUB_FAIL_UPLOAD=1
(cd "$tmp/rel" && bash "$script" v9.9.9 vdb-linux-amd64) >/dev/null 2>&1
