...
run() { # run <prefix-dir> [env...]  -> installer output, exit code in $?
	...
	env ODB_BASE_URL="file://$rel" ODB_PREFIX="$prefix" "$@" sh "$installer" 2>&1
}

# 1. A good release installs and reports each file as verified.
p="$tmp/p1"; out="$(run "$p")"; code=$?
if [ "$code" = 0 ] && [ -x "$p/bin/odb" ] && grep -q "verified $asset" <<<"$out"; then
...
printf '#!/bin/sh\necho "evil"\n' > "$bad_rel/$asset"
p="$tmp/p2"; out="$(env ODB_BASE_URL="file://$bad_rel" ODB_PREFIX="$p" sh "$installer" 2>&1)"; code=$?
if [ "$code" != 0 ] && [ ! -e "$p/bin/odb" ] && grep -q "checksum mismatch" <<<"$out"; then
	ok "a tampered download is refused and nothing is installed"
else
	bad "a tampered download is refused (exit $code, odb present: $([ -e "$p/bin/odb" ] && echo yes || echo no))"
fi

# 3. A release without SHA256SUMS must not install.
no_sums="$tmp/nosums"; cp -r "$rel" "$no_sums"; rm -f "$no_sums/SHA256SUMS"
p="$tmp/p3"; out="$(env ODB_BASE_URL="file://$no_sums" ODB_PREFIX="$p" sh "$installer" 2>&1)"; code=$?
if [ "$code" != 0 ] && [ ! -e "$p/bin/odb" ] && grep -q "could not fetch SHA256SUMS" <<<"$out"; then
...
p="$tmp/p4"; out="$(env ODB_BASE_URL="file://$gap" ODB_PREFIX="$p" sh "$installer" 2>&1)"; code=$?
if [ "$code" != 0 ] && [ ! -e "$p/bin/odb" ] && grep -q "not listed in SHA256SUMS" <<<"$out"; then
	ok "an asset missing from SHA256SUMS is refused"
else
	bad "an asset missing from SHA256SUMS is refused (exit $code)"
fi

# 5. ODB_NO_VERIFY=1 installs the tampered build deliberately.
p="$tmp/p5"; out="$(env ODB_BASE_URL="file://$bad_rel" ODB_PREFIX="$p" ODB_NO_VERIFY=1 sh "$installer" 2>&1)"; code=$?
if [ "$code" = 0 ] && [ -x "$p/bin/odb" ]; then
	ok "ODB_NO_VERIFY=1 installs without checking"
else
	bad "ODB_NO_VERIFY=1 installs without checking (exit $code)"
fi

# 6. A local build (ODB_DIST) still installs — there is no release to check.
p="$tmp/p6"; out="$(env ODB_DIST="$rel" ODB_PREFIX="$p" sh "$installer" 2>&1)"; code=$?
if [ "$code" = 0 ] && [ -x "$p/bin/odb" ]; then
	ok "a local ODB_DIST build still installs"
else
	bad "a local ODB_DIST build still installs (exit $code)"; tail -n 5 <<<"$out" | sed 's/^/      /'
fi
