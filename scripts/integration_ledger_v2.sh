...
pg() { local c="$1"; shift; sudo docker exec "$c" psql -U oxyndb -d oxyndb -tAc "$*" 2>/dev/null; }
...
main_ready() {
  ...
  for i in $(seq 1 60); do
    [ "$(sudo docker exec -e PGPASSWORD=oxyndb oxyn-main psql -h localhost -U oxyndb -d oxyndb -tAc 'SELECT 1' 2>/dev/null)" = "1" ] && return 0
    sleep 1
  done
...
# pass OXYNDB_API_KEY=... in the env arguments to use a different key.
mcp_call() {
  local tool="$1" args="$2"; shift 2
  printf '%s\n' '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}' \
    "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"$tool\",\"arguments\":$args}}" \
    | env OXYNDB_API_KEY="$KEY" "$@" "$S" mcp 2>/dev/null \
...
USER_EMAIL="v2test@oxyndb.dev"
printf 'password123\n' | $S user create "$USER_EMAIL" >/dev/null 2>&1 || true
KEY="$($S apikey create "$USER_EMAIL" v2 2>/dev/null | grep -o 'odb_[A-Za-z0-9_-]*')"
...
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2keep" >/dev/null
gw "$KEY" main "CREATE TABLE v2keep(x int)" >/dev/null
assert_eq "guardrail error text is unchanged" \
  "$(gw "$KEY" main "DROP TABLE v2keep" | grep -c 'OxynDB guardrail: DROP TABLE is blocked by policy (set odb.allow_destructive=on to override)')" "1"
...
SQLHASH="$(pg oxyn-main "SELECT odb._ledger_hash(ROW(42,'2026-01-02 03:04:05+00','priya','human','psql','s1','main','CREATE TABLE','table','public.t','CREATE TABLE t()','APPLIED',NULL,'abc',NULL)::odb.schema_ledger)")"
...
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2pit; CREATE TABLE v2pit(id int); INSERT INTO v2pit SELECT generate_series(1,3);" >/dev/null
$S backup create >/dev/null 2>&1
pg oxyn-main "SELECT pg_switch_wal();" >/dev/null; sleep 3
RESTORE_OUT="$($S restore --to latest 2>&1)"
assert_eq "PITR restores 3 rows" "$(pg oxyn-restore 'SELECT count(*) FROM v2pit')" "3"
assert_eq "restore no longer prints a hardcoded password" "$(echo "$RESTORE_OUT" | grep -c 'PGPASSWORD=oxyndb')" "0"
assert_eq "restore container has no minioadmin credentials" \
  "$(sudo docker inspect -f '{{range .Config.Env}}{{println .}}{{end}}' oxyn-restore 2>/dev/null | grep -c '=minioadmin$')" "0"
sudo docker rm -f oxyn-restore >/dev/null 2>&1

echo "### 1b. H2: only superusers and odb_admin members may override the guardrail"
assert_eq "ledger upgrade succeeds on main" "$($S ledger upgrade main >/dev/null 2>&1; echo $?)" "0"
assert_eq "override check is installed" "$(pg oxyn-main "SELECT to_regprocedure('odb._may_override()') IS NOT NULL")" "t"
assert_eq "odb_admin role exists" "$(pg oxyn-main "SELECT count(*) FROM pg_roles WHERE rolname='odb_admin'")" "1"
gw "$KEY" main "CREATE TABLE IF NOT EXISTS v2guard(x int)" >/dev/null
assert_eq "non-admin override is refused" \
  "$(gw "$KEY" main "SET odb.allow_destructive=on; DROP TABLE v2guard" | grep -c 'blocked by policy')" "1"
assert_eq "refused override leaves the table in place" "$(pg oxyn-main "SELECT to_regclass('public.v2guard') IS NOT NULL")" "t"
assert_eq "refusal carries the odb_admin hint" \
  "$(gw "$KEY" main "SET odb.allow_destructive=on; DROP TABLE v2guard" | grep -c 'odb_admin')" "1"
...
gw "$KEY" main "SET odb.allow_destructive=on; DROP TABLE v2guard" >/dev/null
assert_eq "admin override drops the table" "$(pg oxyn-main "SELECT to_regclass('public.v2guard') IS NULL")" "t"
$S admin revoke "$USER_EMAIL" --branch main >/dev/null 2>&1
gw "$KEY" main "CREATE TABLE IF NOT EXISTS v2guard(x int)" >/dev/null
assert_eq "revoked user is refused again" \
  "$(gw "$KEY" main "SET odb.allow_destructive=on; DROP TABLE v2guard" | grep -c 'blocked by policy')" "1"
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2guard; DROP TABLE IF EXISTS v2keep; DROP TABLE IF EXISTS v2pit" >/dev/null
assert_eq "superuser override still works (imports/pipelines path)" "$(pg oxyn-main "SELECT to_regclass('public.v2keep') IS NULL")" "t"
...
assert_eq "agent change attributed to the agent" \
  "$(pg oxyn-agent-v2itest "SELECT actor||'/'||actor_kind FROM odb.schema_ledger WHERE command_tag='CREATE TABLE' AND object_identity='public.a'")" "agent-v2itest/agent"
...
assert_eq "agent cannot override the guardrail" \
  "$(psql "$DSN" -tAc "SET odb.allow_destructive=on; DROP TABLE a" 2>&1 | grep -c 'blocked by policy')" "1"
assert_eq "MCP list_branches does not expose a password" \
  "$(mcp_call list_branches '{}' | grep -c 'postgresql://agent-v2itest@')" "1"
assert_eq "MCP run_sql runs as the client role" "$(mcp_call run_sql '{"sql":"SELECT session_user"}' | grep -c 'odbclient')" "1"
...
assert_eq "OXYNDB_MCP_SUPERUSER=1 restores the old role" \
  "$(mcp_call run_sql '{"sql":"SELECT session_user"}' OXYNDB_MCP_SUPERUSER=1 | grep -c 'oxyndb')" "1"
# K6: the MCP server acts as an API key's account, and refuses to run without one.
assert_eq "odb mcp without a key exits non-zero" \
  "$(printf '' | $S mcp >/dev/null 2>/tmp/mcpnokey.log; echo $?)" "1"
assert_eq "…and says how to make one" "$(grep -c 'odb apikey create' /tmp/mcpnokey.log)" "1"
assert_eq "…and names the environment variable a client config sets" "$(grep -c 'OXYNDB_API_KEY' /tmp/mcpnokey.log)" "1"
assert_eq "odb mcp with an invalid key refuses to run" \
  "$(printf '' | OXYNDB_API_KEY=odb_notarealkey $S mcp >/dev/null 2>&1; echo $?)|$(printf '' | OXYNDB_API_KEY=odb_notarealkey $S mcp 2>&1 >/dev/null | grep -c 'not valid')" "1|1"
...
assert_eq "MCP run_sql records the key's account as the actor" \
  "$(mcp_call run_sql '{"branch":"v2h3","sql":"CREATE TABLE h3mcp(x int)"}' >/dev/null; pg oxyn-v2h3 "SELECT DISTINCT actor||'/'||actor_kind FROM odb.schema_ledger WHERE object_identity='public.h3mcp'")" "$USER_EMAIL/agent"
...
mcp_call run_sql '{"sql":"CREATE TABLE h3scoped(x int)"}' OXYNDB_API_KEY="$AKEY" >/dev/null
assert_eq "a scoped key's MCP call lands in its own branch, not main" \
  "$(pg oxyn-agent-v2itest "SELECT to_regclass('public.h3scoped') IS NOT NULL")|$(pg oxyn-main "SELECT to_regclass('public.h3scoped') IS NULL")" "t|t"
assert_eq "…naming another branch is refused" \
  "$(mcp_call run_sql '{"branch":"main","sql":"SELECT 1"}' OXYNDB_API_KEY="$AKEY" | grep -c 'limited to branch')" "1"
assert_eq "…and so are the tools that reach past one branch" \
  "$(mcp_call list_branches '{}' OXYNDB_API_KEY="$AKEY" | grep -c 'limited to branch')|$(mcp_call create_branch '{"agent_id":"other"}' OXYNDB_API_KEY="$AKEY" | grep -c 'limited to branch')" "1|1"

# H3: actor_kind follows the role, so a client cannot record itself as the other
# kind. odb.actor_kind is an ordinary session setting, and used to be believed.
PGPASSWORD="$KEY" psql "$GATEWAY/v2h3" -q -c "SET odb.actor_kind='agent'; CREATE TABLE h3human(x int);" >/dev/null 2>&1
assert_eq "a human claiming to be an agent is still recorded as human" \
  "$(pg oxyn-v2h3 "SELECT DISTINCT actor||'/'||actor_kind FROM odb.schema_ledger WHERE object_identity='public.h3human'")" "$USER_EMAIL/human"
psql "$DSN" -q -c "SET odb.actor_kind='human'; CREATE TABLE h3agent(x int);" >/dev/null 2>&1
assert_eq "an agent claiming to be human is still recorded as an agent" \
  "$(pg oxyn-agent-v2itest "SELECT DISTINCT actor||'/'||actor_kind FROM odb.schema_ledger WHERE object_identity='public.h3agent'")" "agent-v2itest/agent"
...
pgerr() { local c="$1"; shift; sudo docker exec "$c" psql -U oxyndb -d oxyndb -tAc "$*" 2>&1; }
assert_eq "ledger 2.0 is installed on main at start" "$(pg oxyn-main "SELECT odb.ledger_v2_version()")" "2.0-phase3"
...
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2cap; DROP TABLE IF EXISTS v2off; DROP TABLE IF EXISTS v2safe" >/dev/null
PGOPTIONS='-c odb.task=task-42 -c odb.call_hash=abc123' PGPASSWORD="$KEY" psql "$GATEWAY/main" -qc "CREATE TABLE v2cap(x int)" >/dev/null 2>&1
CAPID="$(pg oxyn-main "SELECT max(id) FROM odb.schema_ledger WHERE command_tag='CREATE TABLE' AND object_identity='public.v2cap'")"
assert_eq "every new ledger row gets a capture row" "$(pg oxyn-main "SELECT count(*) FROM odb.ledger_ext WHERE ledger_id=$CAPID")" "1"
assert_eq "capture records the transaction and WAL position" \
  "$(pg oxyn-main "SELECT (xid IS NOT NULL AND lsn IS NOT NULL)::text FROM odb.ledger_ext WHERE ledger_id=$CAPID")" "true"
assert_eq "capture records task and call hash from the session" \
  "$(pg oxyn-main "SELECT task_id||'/'||call_hash FROM odb.ledger_ext WHERE ledger_id=$CAPID")" "task-42/abc123"
assert_eq "capture hash verifies" \
  "$(pg oxyn-main "SELECT (e.ext_hash = odb._ext_hash(e))::text FROM odb.ledger_ext e WHERE ledger_id=$CAPID")" "true"
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE v2cap" >/dev/null
assert_eq "override use is recorded" \
  "$(pg oxyn-main "SELECT e.override_used::text FROM odb.ledger_ext e JOIN odb.schema_ledger s ON s.id=e.ledger_id WHERE s.command_tag='DROP TABLE' AND s.object_identity='public.v2cap' ORDER BY s.id DESC LIMIT 1")" "true"
# Fail-safe: a capture that errors must never block the user's DDL.
pg oxyn-main "ALTER TABLE odb.ledger_ext ADD CONSTRAINT v2_test_fail CHECK (false) NOT VALID" >/dev/null
SAFEMARK="$(pg oxyn-main "SELECT coalesce(max(id), 0) FROM odb.schema_ledger")"   # count only this run's rows
gw "$KEY" main "CREATE TABLE v2safe(x int)" >/dev/null
assert_eq "a failing capture does not block DDL" "$(pg oxyn-main "SELECT to_regclass('public.v2safe') IS NOT NULL")" "t"
assert_eq "the base ledger still records that DDL" \
  "$(pg oxyn-main "SELECT count(*) FROM odb.schema_ledger WHERE command_tag='CREATE TABLE' AND object_identity='public.v2safe' AND id > $SAFEMARK")" "1"
pg oxyn-main "ALTER TABLE odb.ledger_ext DROP CONSTRAINT v2_test_fail" >/dev/null
# Kill switch: odb.v2=off stops capture for new sessions.
pg oxyn-main "ALTER DATABASE oxyndb SET odb.v2 = 'off'" >/dev/null
gw "$KEY" main "CREATE TABLE v2off(x int)" >/dev/null
OFFID="$(pg oxyn-main "SELECT max(id) FROM odb.schema_ledger WHERE object_identity='public.v2off'")"
assert_eq "kill switch disables capture" "$(pg oxyn-main "SELECT count(*) FROM odb.ledger_ext WHERE ledger_id=$OFFID")" "0"
pg oxyn-main "ALTER DATABASE oxyndb RESET odb.v2" >/dev/null
# …but it isn't a client's to flip: a session SET is honoured only for a superuser.
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2nocap; DROP TABLE IF EXISTS v2nocap_admin; DROP TABLE IF EXISTS v2nocap_su" >/dev/null
gw "$KEY" main "SET odb.v2 = 'off'; CREATE TABLE v2nocap(x int)" >/dev/null
NCID="$(pg oxyn-main "SELECT max(id) FROM odb.schema_ledger WHERE object_identity='public.v2nocap'")"
assert_eq "a client's own SET odb.v2 = 'off' does not skip capture" \
  "$(pg oxyn-main "SELECT count(*) FROM odb.ledger_ext WHERE ledger_id=${NCID:-0}")" "1"
$S admin grant "$USER_EMAIL" --branch main >/dev/null 2>&1
gw "$KEY" main "SET odb.v2 = 'off'; CREATE TABLE v2nocap_admin(x int)" >/dev/null
NCID="$(pg oxyn-main "SELECT max(id) FROM odb.schema_ledger WHERE object_identity='public.v2nocap_admin'")"
assert_eq "…nor does a odb_admin member's" \
  "$(pg oxyn-main "SELECT count(*) FROM odb.ledger_ext WHERE ledger_id=${NCID:-0}")" "1"
$S admin revoke "$USER_EMAIL" --branch main >/dev/null 2>&1
pg oxyn-main "SET odb.v2 = 'off'; CREATE TABLE v2nocap_su(x int)" >/dev/null
NCID="$(pg oxyn-main "SELECT max(id) FROM odb.schema_ledger WHERE object_identity='public.v2nocap_su'")"
assert_eq "a superuser's session can still switch capture off" \
  "$(pg oxyn-main "SELECT count(*) FROM odb.ledger_ext WHERE ledger_id=${NCID:-0}")" "0"
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2nocap; DROP TABLE IF EXISTS v2nocap_admin; DROP TABLE IF EXISTS v2nocap_su" >/dev/null
assert_eq "capture table is append-only" \
  "$(pgerr oxyn-main "DELETE FROM odb.ledger_ext WHERE ledger_id=$CAPID" | grep -c 'append-only')" "1"
assert_eq "clients cannot write capture rows" \
  "$(gw "$KEY" main "INSERT INTO odb.ledger_ext(ledger_id) VALUES (-1)" | grep -c 'permission denied')" "1"
assert_eq "base hash chain still verifies after 2.0 activity" "$($S ledger verify 2>&1 | grep -c 'ledger intact')" "1"
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2off; DROP TABLE IF EXISTS v2safe" >/dev/null

echo "### 3. checkpoints, anchors and the independent verifier"
VERIFY="${OXYNDB_VERIFY_BIN:-/tmp/odb-verify}"
ANCH="${OXYNDB_ANCHOR_DIR:-$HOME/.oxyndb/anchors}"
assert_eq "checkpoints table is installed" "$(pg oxyn-main "SELECT to_regclass('odb.ledger_checkpoints') IS NOT NULL")" "t"
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2cp" >/dev/null
...
assert_eq "anchor uses the published format" \
  "$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1]))["format"])' "$CP_FILE" 2>/dev/null)" "oxyndb-ledger-anchor/1"
...
assert_eq "database refuses a checkpoint that doesn't continue the sequence" \
  "$(pgerr oxyn-main "INSERT INTO odb.ledger_checkpoints(from_id,to_id,entry_count,merkle_root,prev_root,algorithm) VALUES (1,1,1,'x','','t')" | grep -c 'must start at')" "1"
assert_eq "checkpoints are append-only" "$(pgerr oxyn-main "DELETE FROM odb.ledger_checkpoints" | grep -c 'append-only')" "1"
...
fresh_branch() {
  ...
  for t in v2i_a v2i_b v2i_c; do pg "oxyn-$1" "CREATE TABLE $t(x int)" >/dev/null; done
  $S ledger checkpoint "$1" >/dev/null 2>&1
}
integrity_state() { $S ledger integrity "$1" 2>&1 | head -1 | grep -o 'INTACT\|TAMPERED'; }

fresh_branch v2int
V2DSN="postgresql://oxyndb:$KEY@127.0.0.1:6432/v2int"
...
assert_eq "odb-verify on the live database agrees (exit 0)" "$?" "0"
$S ledger export v2int > /tmp/v2int.jsonl 2>/dev/null
assert_eq "export has one line per ledger entry" "$(wc -l < /tmp/v2int.jsonl | tr -d ' ')" "$(pg oxyn-v2int "SELECT count(*) FROM odb.schema_ledger")"
"$VERIFY" --export /tmp/v2int.jsonl --anchors "$ANCH/v2int" >/dev/null 2>&1
assert_eq "odb-verify on the exported file agrees (exit 0)" "$?" "0"
...
UPDATE odb.schema_ledger SET statement = statement || ' -- forged' WHERE id = (SELECT max(id) FROM odb.schema_ledger);
DO \$\$ DECLARE r record; prev text := ''; BEGIN
  FOR r IN SELECT id FROM odb.schema_ledger WHERE row_hash IS NOT NULL ORDER BY id LOOP
    UPDATE odb.schema_ledger SET prev_hash = prev WHERE id = r.id;
    UPDATE odb.schema_ledger s SET row_hash = odb._ledger_hash(s) WHERE s.id = r.id RETURNING s.row_hash INTO prev;
  END LOOP; END \$\$;"
pg oxyn-v2int "$FORGE" >/dev/null
...
"$VERIFY" --dsn "$V2DSN" --anchors "$ANCH/v2int" >/dev/null 2>&1
assert_eq "forged edit: odb-verify exits 1" "$?" "1"

fresh_branch v2int
pg oxyn-v2int "SET session_replication_role = replica; DELETE FROM odb.schema_ledger WHERE id = (SELECT max(id) FROM odb.schema_ledger)" >/dev/null
...
fresh_branch v2int
pg oxyn-v2int "SET session_replication_role = replica; DELETE FROM odb.schema_ledger" >/dev/null
assert_eq "wiped ledger: integrity is TAMPERED" "$(integrity_state v2int)" "TAMPERED"

fresh_branch v2int
pg oxyn-v2int "SET session_replication_role = replica; DELETE FROM odb.ledger_checkpoints" >/dev/null
...
$S stop >/dev/null 2>&1; sleep 1
OXYNDB_CHECKPOINT_INTERVAL=15s $S start >/dev/null 2>&1; main_ready
BEFORE="$(count_main "SELECT count(*) FROM odb.ledger_checkpoints")"
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2sched" >/dev/null
gw "$KEY" main "CREATE TABLE v2sched(x int)" >/dev/null
AFTER="$BEFORE"
for i in $(seq 1 12); do
  AFTER="$(count_main "SELECT count(*) FROM odb.ledger_checkpoints")"
...
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2sched; DROP TABLE IF EXISTS v2cp" >/dev/null
$S stop >/dev/null 2>&1; sleep 1
$S start >/dev/null 2>&1; sleep 5; main_ready
...
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2bb_keep; DROP TABLE IF EXISTS v2bb_old; DROP TABLE IF EXISTS v2bb_target; DROP TABLE IF EXISTS v2bb_after" >/dev/null
...
sleep 1
pg oxyn-main "SET odb.v2 = 'off'; CREATE TABLE v2bb_old(x int)" >/dev/null       # an entry without capture (time fallback)
OLDID="$(pg oxyn-main "SELECT max(id) FROM odb.schema_ledger WHERE object_identity = 'public.v2bb_old'")"
sleep 1
gw "$KEY" main "CREATE TABLE v2bb_target(x int)" >/dev/null
TID="$(pg oxyn-main "SELECT max(id) FROM odb.schema_ledger WHERE object_identity = 'public.v2bb_target'")"
...
assert_eq "the target entry has a captured transaction id" \
  "$(pg oxyn-main "SELECT (xid IS NOT NULL)::text FROM odb.ledger_ext WHERE ledger_id = $TID")" "true"
assert_eq "the fallback entry has none" \
  "$(pg oxyn-main "SELECT count(*) FROM odb.ledger_ext WHERE ledger_id = $OLDID")" "0"
assert_eq "odb ledger entries lists the entry id" "$($S ledger entries --limit 10 | awk '{print $1}' | grep -cx "$TID")" "1"
...
assert_eq "the new branch is running" "$(sudo docker inspect -f '{{.State.Status}}' oxyn-v2bb 2>/dev/null)" "running"
assert_eq "rows written before the change are there (including after the backup)" "$(pg oxyn-v2bb 'SELECT count(*) FROM v2bb_keep')" "7"
assert_eq "an earlier change is there" "$(pg oxyn-v2bb "SELECT count(*) FROM pg_tables WHERE tablename = 'v2bb_old'")" "1"
assert_eq "the change itself is not" "$(pg oxyn-v2bb "SELECT count(*) FROM pg_tables WHERE tablename = 'v2bb_target'")" "0"
assert_eq "later changes are not" "$(pg oxyn-v2bb "SELECT count(*) FROM pg_tables WHERE tablename = 'v2bb_after'")" "0"
assert_eq "its ledger stops before the entry" "$(pg oxyn-v2bb "SELECT count(*) FROM odb.schema_ledger WHERE id >= $TID")" "0"
assert_eq "no recovery settings are left behind" \
  "$(pg oxyn-v2bb "SELECT count(*) FROM pg_file_settings WHERE name LIKE 'recovery_target%' OR name = 'restore_command'")" "0"
...
assert_eq "main is untouched" \
  "$(pg oxyn-main "SELECT count(*) FROM pg_tables WHERE tablename IN ('v2bb_target','v2bb_after')")|$(pg oxyn-main 'SELECT count(*) FROM v2bb_keep')" "2|8"
...
BID="$(pg oxyn-main "SELECT max(id) FROM odb.schema_ledger WHERE status = 'BLOCKED'")"
assert_eq "a BLOCKED entry is refused" "$($S ledger branch-before "$BID" --as v2bb-x 2>&1 | grep -c 'BLOCKED')" "1"
assert_eq "a refused request leaves nothing behind" "$(sudo docker inspect oxyn-v2bb-x >/dev/null 2>&1 || echo none)" "none"
...
assert_eq "MCP lists branch_before_change" \
  "$(printf '%s\n' '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | env OXYNDB_API_KEY="$KEY" "$S" mcp 2>/dev/null | grep -c 'branch_before_change')" "1"
...
assert_eq "REST branch holds main before the change" \
  "$(pg oxyn-v2bb-rest "SELECT count(*) FROM pg_tables WHERE tablename = 'v2bb_target'")|$(pg oxyn-v2bb-rest 'SELECT count(*) FROM v2bb_keep')" "0|7"
...
assert_eq "time fallback excludes the change and keeps earlier rows" \
  "$(pg oxyn-v2bb-time "SELECT count(*) FROM pg_tables WHERE tablename = 'v2bb_old'")|$(pg oxyn-v2bb-time 'SELECT count(*) FROM v2bb_keep')" "0|7"

$S branch delete v2bb >/dev/null 2>&1
assert_eq "the branch deletes like any other" "$(sudo docker inspect oxyn-v2bb >/dev/null 2>&1 || echo gone)" "gone"
...
assert_eq "odb blackbox verify matches odb ledger verify" "$($S blackbox verify main 2>&1)" "$($S ledger verify main 2>&1)"
assert_eq "odb blackbox entries matches odb ledger entries" "$($S blackbox entries --limit 5 2>&1)" "$($S ledger entries --limit 5 2>&1)"
assert_eq "odb blackbox integrity matches odb ledger integrity" \
...
assert_eq "MCP lists both the Blackbox and the original tool names" \
  "$(printf '%s\n' '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | env OXYNDB_API_KEY="$KEY" "$S" mcp 2>/dev/null | python3 -c 'import sys,json; n={t["name"] for t in json.load(sys.stdin)["result"]["tools"]}; print(all(x in n for x in ["verify_blackbox","blackbox_integrity","blackbox_entries","verify_ledger","ledger_integrity","ledger_entries"]))')" "True"
...
TODAY="$(pg oxyn-main "SELECT to_char(now(),'YYYY-MM-DD')")"
...
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS $PT" >/dev/null

echo "### 6. Blackbox policy gate (warn / block — docs/policy-errors.md)"
# gwv: psql through the gateway with SQLSTATEs shown ("NOTICE:  ODB02: …").
...
reset_rules
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2pol; DROP TABLE IF EXISTS v2pol_forbidden; DROP TABLE IF EXISTS v2pol_ok" >/dev/null
gw "$KEY" main "CREATE TABLE v2pol(a int, b int, c text)" >/dev/null

assert_eq "the four built-in rules are installed, all warn" \
  "$(pg oxyn-main "SELECT string_agg(rule_id||':'||action, ',' ORDER BY rule_id) FROM odb.policy_rules WHERE builtin")" \
...
assert_eq "the gate fires after the guardrail (name order)" \
  "$(pg oxyn-main "SELECT string_agg(evtname, ',' ORDER BY evtname) FROM pg_event_trigger WHERE evtname IN ('odb_guard_start','odb_policy_start')")" \
  "odb_guard_start,odb_policy_start"
assert_eq "odb policy lists the rules" "$($S policy list | grep -c 'alter-column-type\|drop-column\|drop-index\|grant-to-public')" "4"

EVB="$(pg oxyn-main "SELECT coalesce(max(id),0) FROM odb.ledger_policy_evaluations")"
OUT="$(gwv "$KEY" main "ALTER TABLE v2pol ALTER COLUMN a TYPE bigint")"
assert_eq "warn: the statement runs" \
  "$(pg oxyn-main "SELECT data_type FROM information_schema.columns WHERE table_name='v2pol' AND column_name='a'")" "bigint"
assert_eq "warn: a NOTICE with SQLSTATE ODB02" "$(echo "$OUT" | grep -c '^NOTICE:  ODB02: Blackbox policy warning: .*(rule alter-column-type)')" "1"
...
assert_eq "warn: the evaluation is recorded" \
  "$(pg oxyn-main "SELECT count(*) FROM odb.ledger_policy_evaluations WHERE id > $EVB AND rule_id='alter-column-type' AND action='warn'")" "1"
gw "$KEY" main "CREATE INDEX v2pol_b ON v2pol(b)" >/dev/null
assert_eq "warn: DROP INDEX" "$(gwv "$KEY" main "DROP INDEX v2pol_b" | grep -c 'ODB02: .*(rule drop-index)')" "1"
assert_eq "warn: GRANT … TO PUBLIC" "$(gwv "$KEY" main "GRANT SELECT ON v2pol TO PUBLIC" | grep -c 'ODB02: .*(rule grant-to-public)')" "1"
assert_eq "an ordinary change triggers nothing" "$(gwv "$KEY" main "ALTER TABLE v2pol ADD COLUMN d int" | grep -c 'ODB0')" "0"
...
assert_eq "odb policy block sets the action" "$(pg oxyn-main "SELECT action FROM odb.policy_rules WHERE rule_id='drop-column'")" "block"
OUT="$(gwv "$KEY" main "ALTER TABLE v2pol DROP COLUMN c")"
assert_eq "block: an ERROR with SQLSTATE ODB01" "$(echo "$OUT" | grep -c '^ERROR:  ODB01: Blackbox policy: .*(rule drop-column)')" "1"
assert_eq "block: the statement did not run" \
  "$(pg oxyn-main "SELECT count(*) FROM information_schema.columns WHERE table_name='v2pol' AND column_name='c'")" "1"
BD="$(echo "$OUT" | detail)"
assert_eq "block: DETAIL is the contract JSON" \
  ...
  "1 drop-column block odb_admin None True True"
...
assert_eq "block: recorded as a BLOCKED Blackbox entry (risk policy) that survived the rollback" \
  "$(pg oxyn-main "SELECT status||'/'||risk||'/'||command_tag FROM odb.schema_ledger WHERE id=${BBID:-0}")" "BLOCKED/policy/ALTER TABLE"
assert_eq "block: its evaluation survived the rollback" "$(pg oxyn-main "SELECT action FROM odb.ledger_policy_evaluations WHERE blackbox_id=${BBID:-0}")" "block"
assert_eq "block: the HINT names the override" "$(echo "$OUT" | grep -c "^HINT:  .*odb.policy_allow = 'drop-column'")" "1"
assert_eq "block: a non-admin's odb.policy_allow is ignored" \
  "$(gwv "$KEY" main "SET odb.policy_allow = 'drop-column'; ALTER TABLE v2pol DROP COLUMN c" | grep -c 'ODB01')" "1"
assert_eq "block: turning odb.v2 off in the session does not bypass it" \
  "$(gwv "$KEY" main "SET odb.v2 = 'off'; ALTER TABLE v2pol DROP COLUMN c" | grep -c 'ODB01')" "1"
...
$S admin grant "$USER_EMAIL" --branch main >/dev/null 2>&1
assert_eq "admin: odb.allow_destructive does not override a policy rule" \
  "$(gwv "$KEY" main "SET odb.allow_destructive = on; ALTER TABLE v2pol DROP COLUMN c" | grep -c 'ODB01')" "1"
EVB="$(pg oxyn-main "SELECT coalesce(max(id),0) FROM odb.ledger_policy_evaluations")"
OUT="$(gwv "$KEY" main "SET odb.policy_allow = 'drop-column'; ALTER TABLE v2pol DROP COLUMN c")"
assert_eq "admin: odb.policy_allow overrides that rule" \
  "$(pg oxyn-main "SELECT count(*) FROM information_schema.columns WHERE table_name='v2pol' AND column_name='c'")|$(echo "$OUT" | grep -c 'ODB01')" "0|0"
assert_eq "admin: the override is recorded as allowed" \
  "$(pg oxyn-main "SELECT count(*) FROM odb.ledger_policy_evaluations WHERE id > $EVB AND rule_id='drop-column' AND action='allowed'")" "1"
assert_eq "admin: the change's capture row records the override" \
  "$(pg oxyn-main "SELECT e.override_used FROM odb.ledger_ext e JOIN odb.schema_ledger s ON s.id=e.ledger_id WHERE s.object_identity='public.v2pol' AND s.command_tag='ALTER TABLE' ORDER BY s.id DESC LIMIT 1")" "t"
...
assert_eq "REST: the change took effect and names who made it" \
  "$(pg oxyn-main "SELECT action||'/'||updated_by FROM odb.policy_rules WHERE rule_id='drop-column'")" "warn/$USER_EMAIL"
...
# L4/H8: the console used to log every user in as the shared odbclient role,
# which is never in odb_admin, so nobody could override the guardrail from it.
...
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2console" >/dev/null
...
assert_eq "console: with the override an admin's DROP goes through" \
  "$(cq 'DROP TABLE v2console' 1 | grep -c '"error"')|$(pg oxyn-main "SELECT to_regclass('public.v2console') IS NULL")" "0|t"
assert_eq "REST: granting an account that doesn't exist is 404" \
  "$(curl -sk -o /dev/null -w '%{http_code}' -X POST -H "$AUTH" -d '{"email":"nobody@oxyndb.dev"}' "$API/api/branches/main/admins")" "404"
assert_eq "REST: revoking someone who isn't an admin is 404" \
  "$(curl -sk -o /dev/null -w '%{http_code}' -X DELETE -H "$AUTH" "$API/api/branches/main/admins/nobody@oxyndb.dev")" "404"
# A policy rule (ODB01) is overridden per rule, not by allow_destructive; the
...
$S policy block drop-column >/dev/null 2>&1
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2console2" >/dev/null
cq "CREATE TABLE v2console2(a int, b int, c int)" >/dev/null
assert_eq "console: allow_destructive does not override a policy rule" "$(cq 'ALTER TABLE v2console2 DROP COLUMN b' 1 | grep -c 'ODB01')" "1"
assert_eq "console: allow_rules overrides that rule for an admin" \
  "$(cqr 'ALTER TABLE v2console2 DROP COLUMN b' drop-column | grep -c '"error"')|$(pg oxyn-main "SELECT count(*) FROM information_schema.columns WHERE table_name='v2console2' AND column_name='b'")" "0|0"
...
$S admin revoke "$USER_EMAIL" --branch main >/dev/null 2>&1
assert_eq "console: allow_rules is ignored for a non-admin" "$(cqr 'ALTER TABLE v2console2 DROP COLUMN c' drop-column | grep -c 'ODB01')" "1"
...
assert_eq "clients can't change rules directly" "$(gw "$KEY" main "UPDATE odb.policy_rules SET action='warn'" | grep -c 'permission denied')" "1"

# Fail-safe: when an evaluation can't be recorded, a warn rule never blocks, and
# a block still refuses (with evaluation_id null).
pg oxyn-main "ALTER TABLE odb.ledger_policy_evaluations ADD CONSTRAINT v2pol_fail CHECK (false) NOT VALID" >/dev/null
...
assert_eq "fail-safe: a warn rule that can't record lets the statement run" \
  "$(pg oxyn-main "SELECT data_type FROM information_schema.columns WHERE table_name='v2pol' AND column_name='b'")|$(echo "$OUT" | grep -c 'evaluation skipped')" "bigint|1"
OUT="$(gwv "$KEY" main "ALTER TABLE v2pol DROP COLUMN b")"
assert_eq "fail-safe: a block still refuses, evaluation_id null" \
  "$(echo "$OUT" | grep -c 'ODB01')|$(echo "$OUT" | detail | python3 -c 'import sys,json; print(json.load(sys.stdin)["evaluation_id"])')" "1|None"
$S policy warn drop-column >/dev/null 2>&1
pg oxyn-main "ALTER TABLE odb.ledger_policy_evaluations DROP CONSTRAINT v2pol_fail" >/dev/null

$S policy block drop-column >/dev/null 2>&1
pg oxyn-main "ALTER EVENT TRIGGER odb_policy_start DISABLE" >/dev/null
assert_eq "kill switch: a disabled gate doesn't block" "$(gwv "$KEY" main "ALTER TABLE v2pol DROP COLUMN b" | grep -c 'ODB0')" "0"
pg oxyn-main "ALTER EVENT TRIGGER odb_policy_start ENABLE" >/dev/null
$S policy warn drop-column >/dev/null 2>&1

HB="$(pg oxyn-main "SELECT coalesce(max(id),0) FROM odb.policy_rule_history")"
...
assert_eq "custom block rule refuses a matching statement" \
  "$(gwv "$KEY" main "CREATE TABLE v2pol_forbidden(x int)" | grep -c 'ODB01: .*(rule v2pol-custom)')" "1"
assert_eq "…and nothing else" "$(gwv "$KEY" main "CREATE TABLE v2pol_ok(x int)" | grep -c 'ODB0')" "0"
$S policy disable v2pol-custom >/dev/null 2>&1
assert_eq "a disabled rule doesn't fire" "$(gwv "$KEY" main "CREATE TABLE v2pol_forbidden(x int)" | grep -c 'ODB0')" "0"
assert_eq "rule changes are recorded with who made them" \
  "$(pg oxyn-main "SELECT string_agg(change||':'||changed_by, ',' ORDER BY id) FROM odb.policy_rule_history WHERE id > $HB AND rule_id='v2pol-custom'")" \
  "insert:odb-cli,update:odb-cli"
...
assert_eq "evaluations are append-only" \
  "$(sudo docker exec oxyn-main psql -U oxyndb -d oxyndb -tAc "DELETE FROM odb.ledger_policy_evaluations" 2>&1 | grep -c 'append-only')" "1"

assert_eq "the guardrail's DROP TABLE error is unchanged (42501) and adds no policy notice" \
  "$(gwv "$KEY" main "DROP TABLE v2pol" | grep -c '^ERROR:  42501: OxynDB guardrail: DROP TABLE is blocked by policy (set odb.allow_destructive=on to override)')|$(gwv "$KEY" main "DROP TABLE v2pol" | grep -c 'ODB0')" \
  "1|0"
$S policy block drop-index >/dev/null 2>&1
$S blackbox upgrade main >/dev/null 2>&1
assert_eq "reinstalling keeps rule changes" "$(pg oxyn-main "SELECT action FROM odb.policy_rules WHERE rule_id='drop-index'")" "block"
assert_eq "reinstalling keeps one gate trigger" "$(pg oxyn-main "SELECT count(*) FROM pg_event_trigger WHERE evtname='odb_policy_start'")" "1"
...
reset_rules
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2stm; DROP TABLE IF EXISTS v2stm_a; DROP TABLE IF EXISTS v2stm_b" >/dev/null
gw "$KEY" main "CREATE TABLE v2stm(a int, b int, c int, d int, e int)" >/dev/null

read -r -d '' TOK_SQL <<'SQL'
SELECT string_agg(pos || '=' || body, ' | ' ORDER BY pos) FROM odb._sql_statements(
...
WANT
assert_eq "tokenizer: statements split with comments removed and literals emptied" "$(pg oxyn-main "$TOK_SQL")" "$TOK_WANT"
read -r -d '' MB_SQL <<'SQL'
SELECT count(*) FROM odb._sql_statements('héllo ünïcode; ALTER TABLE t ADD x text DEFAULT ''ü;ß''')
SQL
assert_eq "tokenizer: multibyte text" "$(pg oxyn-main "$MB_SQL")" "2"
read -r -d '' TAG_SQL <<'SQL'
SELECT odb._statement_has_tag('CREATE UNIQUE INDEX CONCURRENTLY i ON t(a)','CREATE INDEX')::text
    || odb._statement_has_tag('CREATE OR REPLACE VIEW v AS SELECT 1','CREATE VIEW')::text
    || odb._statement_has_tag('ALTER VIEW "table" RENAME TO x','ALTER TABLE')::text
SQL
assert_eq "tag matching (modifiers skipped; a quoted \"table\" is not a keyword)" "$(pg oxyn-main "$TAG_SQL")" "truetruefalse"

EVB="$(pg oxyn-main "SELECT coalesce(max(id),0) FROM odb.ledger_policy_evaluations")"
assert_eq "warn: a string literal mentioning the rule doesn't trigger it" \
  "$(gwv "$KEY" main "ALTER TABLE v2stm ADD COLUMN note text DEFAULT 'drop column later'" | grep -c 'ODB02')" "0"
assert_eq "warn: a comment mentioning the rule doesn't trigger it" \
  "$(gwv "$KEY" main "$(printf -- '-- drop column soon\nALTER TABLE v2stm ADD COLUMN n2 int')" | grep -c 'ODB02')" "0"
assert_eq "warn: add then drop in one run warns once, for the drop" \
  "$(gwv "$KEY" main "ALTER TABLE v2stm ADD COLUMN n3 int; ALTER TABLE v2stm DROP COLUMN n3" | grep -c 'ODB02: .*(rule drop-column)')|$(pg oxyn-main "SELECT count(*) FROM odb.ledger_policy_evaluations WHERE id > $EVB AND rule_id='drop-column'")" "1|1"
assert_eq "warn: a comment inside the statement doesn't hide it" \
  "$(gwv "$KEY" main "ALTER TABLE v2stm DROP /* x */ COLUMN n2" | grep -c 'ODB02: .*(rule drop-column)')" "1"
assert_eq "risk: a literal mentioning drop column is not flagged" \
  "$(pg oxyn-main "SELECT status||'/'||coalesce(risk,'-') FROM odb.schema_ledger WHERE object_identity='public.v2stm' AND statement LIKE '%note text%' ORDER BY id DESC LIMIT 1")" "APPLIED/-"
...
assert_eq "risk: in add-then-drop only the drop is flagged" \
  "$(pg oxyn-main "SELECT string_agg(status||'/'||coalesce(risk,'-'), ',' ORDER BY id) FROM (SELECT * FROM odb.schema_ledger WHERE object_identity='public.v2stm' ORDER BY id DESC LIMIT 2) x")" \
...
assert_eq "block: a harmless statement with the phrase in a literal runs" \
  "$(gwv "$KEY" main "ALTER TABLE v2stm ADD COLUMN n5 text DEFAULT 'drop column'" | grep -c 'ERROR')|$(pg oxyn-main "SELECT count(*) FROM information_schema.columns WHERE table_name='v2stm' AND column_name='n5'")" "0|1"
EVB="$(pg oxyn-main "SELECT coalesce(max(id),0) FROM odb.ledger_policy_evaluations")"
S0=$(date +%s)
...
assert_eq "block: add then drop in one run is refused without hanging (it deadlocked before)" \
  "$(echo "$OUT" | grep -c 'ODB01: .*(rule drop-column)')|$([ $(( $(date +%s) - S0 )) -lt 30 ] && echo fast)" "1|fast"
assert_eq "block: …the whole run is rolled back" \
  "$(pg oxyn-main "SELECT count(*) FROM information_schema.columns WHERE table_name='v2stm' AND column_name='n4'")" "0"
assert_eq "block: …the evaluation is still recorded, and the skipped Blackbox entry is announced" \
  "$(pg oxyn-main "SELECT count(*) FROM odb.ledger_policy_evaluations WHERE id > $EVB AND rule_id='drop-column' AND action='block'")|$(echo "$OUT" | grep -c 'not recorded in Blackbox, because this transaction has already written to it')" "1|1"
assert_eq "block: DDL inside a DO block is still caught" \
  "$(gwv "$KEY" main 'DO $$ BEGIN ALTER TABLE v2stm DROP COLUMN d; END $$' | grep -c 'ODB01')" "1"
read -r -d '' BUILT <<'SQL'
DO $$ BEGIN EXECUTE 'ALTER TABLE v2stm DR' || 'OP COLUMN d'; END $$
SQL
assert_eq "block: …including a statement assembled from strings" "$(gwv "$KEY" main "$BUILT" | grep -c 'ODB01')" "1"
assert_eq "a client can't move the statement position" \
  "$(gw "$KEY" main "SELECT odb._statement_texts('start','ALTER TABLE','')" | grep -c 'permission denied')|$(gw "$KEY" main "UPDATE odb.statement_cursor SET pos = 99" | grep -c 'permission denied')" "1|1"
reset_rules
...
assert_eq "guardrail: a blocked DROP TABLE after a write in the same transaction no longer hangs, error unchanged" \
  "$(echo "$OUT" | grep -c '42501: OxynDB guardrail: DROP TABLE is blocked by policy (set odb.allow_destructive=on to override)')|$([ $(( $(date +%s) - S0 )) -lt 30 ] && echo fast)" "1|fast"
assert_eq "no session left waiting" \
  "$(pg oxyn-main "SELECT count(*) FROM pg_stat_activity WHERE datname='oxyndb' AND wait_event_type IN ('Lock','Extension') AND pid <> pg_backend_pid()")" "0"

HB="$(pg oxyn-main "SELECT coalesce(max(id),0) FROM odb.policy_history")"
pg oxyn-main "SET odb.actor = 'v2-audit'; INSERT INTO odb.policy VALUES ('DROP INDEX','flag') ON CONFLICT (op) DO UPDATE SET action = 'flag'" >/dev/null
...
assert_eq "flag: a flagged command runs and is recorded FLAGGED" \
  "$(pg oxyn-main "SELECT status||'/'||risk FROM odb.schema_ledger WHERE command_tag='DROP INDEX' AND object_identity='public.v2stm_a_idx' ORDER BY id DESC LIMIT 1")|$(pg oxyn-main "SELECT to_regclass('public.v2stm_a_idx') IS NULL")" \
  "FLAGGED/drop|t"
assert_eq "an unknown guardrail action is refused" \
  "$(pgerr oxyn-main "UPDATE odb.policy SET action = 'blok' WHERE op = 'DROP INDEX'" | grep -c 'policy_action_valid')" "1"
pg oxyn-main "SET odb.actor = 'v2-audit'; DELETE FROM odb.policy WHERE op = 'DROP INDEX'" >/dev/null
assert_eq "guardrail policy changes are recorded, with who made them" \
  "$(pg oxyn-main "SELECT string_agg(change||':'||coalesce(old_action,'-')||'>'||coalesce(new_action,'-')||':'||changed_by, ',' ORDER BY id) FROM odb.policy_history WHERE id > $HB")" \
  "insert:->flag:v2-audit,delete:flag>-:v2-audit"
assert_eq "…the history is append-only and readable by clients" \
  "$(pgerr oxyn-main "DELETE FROM odb.policy_history" | grep -c 'append-only')|$(gw "$KEY" main "SELECT count(*) > 0 FROM odb.policy_history")" "1|t"
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2stm; DROP TABLE IF EXISTS v2stm_a; DROP TABLE IF EXISTS v2stm_b" >/dev/null
...
$S policy warn drop-column >/dev/null 2>&1
pg oxyn-main "SET odb.allow_destructive=on; DROP TABLE IF EXISTS v2exec; DROP TABLE IF EXISTS v2exec_dry" >/dev/null

assert_eq "agent sessions table is installed" "$(pg oxyn-main "SELECT to_regclass('odb.agent_sessions') IS NOT NULL")" "t"
assert_eq "clients can't write agent sessions" \
  "$(gw "$KEY" main "INSERT INTO odb.agent_sessions(session_id, agent_id) VALUES ('x','y')" | grep -c 'permission denied')" "1"
...
assert_eq "…and sets no session on the agent's branch" \
  "$(pg oxyn-agent-v2plain "SELECT count(*) FROM pg_db_role_setting s, unnest(s.setconfig) c WHERE c LIKE 'odb.session=%'")" "0"
...
assert_eq "the session is recorded on the agent's branch" \
  "$(pg oxyn-agent-v2prov "SELECT agent_id||'/'||task_id||'/'||parent_session_id||'/'||tool FROM odb.agent_sessions WHERE session_id='$SID'")" \
  "agent-v2prov/task-77/sess-parent-1/agent-api"
psql "$(echo "$R" | jf 'd["dsn"]')" -qc "CREATE TABLE v2prov_t(x int)" >/dev/null 2>&1
PID="$(pg oxyn-agent-v2prov "SELECT max(id) FROM odb.schema_ledger WHERE object_identity='public.v2prov_t'")"
assert_eq "the agent's own change carries its session, task and parent" \
  "$(pg oxyn-agent-v2prov "SELECT s.actor||'/'||s.session||'/'||e.task_id||'/'||e.parent_session FROM odb.schema_ledger s JOIN odb.ledger_ext e ON e.ledger_id=s.id WHERE s.id=${PID:-0}")" \
  "agent-v2prov/$SID/task-77/sess-parent-1"
assert_eq "an invalid task id is refused before any branch is made (400)" \
  "$(curl -sk -o /dev/null -w '%{http_code}' -H "$AUTH" -X POST -d "{\"task_id\":\"$LONGID\"}" "$AGENTS/agents/v2bad/branch")|$(sudo docker inspect oxyn-agent-v2bad >/dev/null 2>&1 || echo none)" \
  "400|none"
...
assert_eq "execute_change applies a change" "$(echo "$R" | jf 'd["status"]+" "+d["command"]')|$(pg oxyn-main "SELECT to_regclass('public.v2exec') IS NOT NULL")" "applied CREATE TABLE|t"
...
assert_eq "the entry carries agent, tool, session, task, parent and call hash" \
  "$(pg oxyn-main "SELECT s.actor||'/'||s.actor_kind||'/'||s.tool||'/'||s.session||'/'||e.task_id||'/'||e.parent_session||'/'||e.call_hash FROM odb.schema_ledger s JOIN odb.ledger_ext e ON e.ledger_id=s.id WHERE s.id=${EID:-0}")" \
  "v2test-agent/agent/mcp/$MSID/task-88/sess-p/$HASH"
assert_eq "the MCP session is recorded" \
  "$(pg oxyn-main "SELECT agent_id||'/'||tool||'/'||task_id FROM odb.agent_sessions WHERE session_id='$MSID'")" "v2test-agent/mcp/task-88"
...
assert_eq "execute_change reports a policy warning (ODB02) and still applies" \
  ...
  "applied ODB02:alter-column-type alter-column-type"

$S policy block drop-column >/dev/null 2>&1
R="$(mcp_exec '{"sql":"ALTER TABLE v2exec DROP COLUMN b","task_id":"task-89"}')"
assert_eq "execute_change reports a block with the policy rule (ODB01)" \
  "$(echo "$R" | jf 'd["status"]+" "+d["policy"]["rule_id"]+" "+d["error"]["code"]+" "+str(d["policy"]["blackbox_id"] in d["blackbox_entries"])')" \
  "blocked drop-column ODB01 True"
assert_eq "…and the column is still there" \
  "$(pg oxyn-main "SELECT count(*) FROM information_schema.columns WHERE table_name='v2exec' AND column_name='b'")" "1"
$S policy warn drop-column >/dev/null 2>&1

R="$(mcp_exec '{"sql":"CREATE TABLE v2exec_dry(x int)","dry_run":true}')"
assert_eq "dry_run only previews" "$(echo "$R" | jf 'd["status"]')|$(pg oxyn-main "SELECT to_regclass('public.v2exec_dry') IS NULL")" "preview|t"
...
assert_eq "odb blackbox sessions lists the MCP session" "$($S blackbox sessions main --limit 200 | grep -c "^$MSID ")" "1"
...
$S policy warn drop-column >/dev/null 2>&1
drop_imp() {
  pg oxyn-main "SET odb.allow_destructive=on; DROP VIEW IF EXISTS v2imp_big; DROP VIEW IF EXISTS v2imp_totals; DROP TABLE IF EXISTS v2imp_items; DROP TABLE IF EXISTS v2imp_orders; DROP TABLE IF EXISTS v2imp_mainonly; DROP TABLE IF EXISTS v2diff_main_only" >/dev/null
}
drop_imp
...
assert_eq "the depth limit is reported (as a client)" \
  "$(gw "$KEY" main "SELECT odb.blast_radius('v2imp_orders', NULL, 1)->>'truncated'")" "true"
...
assert_eq "odb branch diff gives the same diff" \
