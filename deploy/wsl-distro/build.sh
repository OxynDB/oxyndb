...
#
# Build the prebuilt OxynDB WSL2 distro image.
#
# Everything that does not depend on the WSL kernel is baked in here: Docker, the
# btrfs tools, the odb engine binary, the Postgres image build context, and the
# container images the stack runs. `odb setup` then only has to import this,
...
#
# Output: dist/oxyndb-distro.tar.gz
#
# Runs on a Linux builder with Docker (CI: ubuntu-latest). Needs
# dist/odb-linux-amd64 to exist first --
...
prepare_rootfs() {
	...
	sudo install -m 0755 "$engine" "$root/usr/local/bin/odb"
	sudo mkdir -p "$root/usr/local/share/oxyndb/docker/postgres"
	sudo cp "$repo/docker/postgres/Dockerfile" \
		"$repo/docker/postgres/restore-entrypoint.sh" \
		"$root/usr/local/share/oxyndb/docker/postgres/"
	sudo chmod +x "$root/usr/local/share/oxyndb/docker/postgres/restore-entrypoint.sh"
...
prepare_images() {
	...
	# a preload under any other name is ignored, and `odb setup` pulls or builds
	# the image anyway — which is what this whole step exists to avoid.
	docker build -t ghcr.io/oxyndb/postgres-walg:16 "$repo/docker/postgres"
	docker pull -q "$minio_image"
	docker pull -q "$mc_image"
	docker save -o "$work/oxyndb-images.tar" \
		ghcr.io/oxyndb/postgres-walg:16 "$minio_image" "$mc_image"
	timer "images" "$t"
}
...
images_dir="$root/usr/local/share/oxyndb/images"
sudo mkdir -p "$images_dir"
sudo mv "$work/oxyndb-images.tar" "$images_dir/oxyndb-images.tar"
sudo chmod 0644 "$images_dir/oxyndb-images.tar"
...
if command -v pigz >/dev/null 2>&1; then
	sudo tar -C "$root" -cpf - . | pigz -6 -p "$(nproc)" > "$out/oxyndb-distro.tar.gz"
else
	echo "    note: pigz not found, falling back to single-threaded gzip"
	sudo tar -C "$root" -czpf "$out/oxyndb-distro.tar.gz" .
fi
sudo chown "$(id -u):$(id -g)" "$out/oxyndb-distro.tar.gz"
timer "repackage" "$t"

echo
ls -la "$out/oxyndb-distro.tar.gz" | awk '{printf "distro image: %s (%.0f MB)\n", $NF, $5/1048576}'
timer "total" "$started"
echo "odb setup imports this, mounts its btrfs storage, and starts."
