...
const (
	...
	// Needed when `vdb gateway --addr` is not the default, or when the Gateway
	// is published somewhere other than localhost.
	EnvGatewayHostPort = "VECTORADB_GATEWAY_HOSTPORT"
...
func AgentRolePassword(branchName string) string {
	...
	m.Write([]byte("vdb-agent-role:" + branchName))
...
// places — the Agent Branch API, `vdb mcp` and the CLI — and only the first has
...
func mintAgentKey(branchName string) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("no account exists yet to own the key — run `vdb start` first")
	}
