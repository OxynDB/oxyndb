...
// a change. A session is recorded in vdb.agent_sessions (internal/ledger/
...
// (vdb.ledger_ext), which checkpoints anchor.
...
// AgentSession is one row of vdb.agent_sessions, with how many Blackbox entries
// carry its id.
...
// vdb.call_hash so a Blackbox entry can be tied to the exact call that caused it.
...
func startSessionSQL(s AgentSession) string {
	return fmt.Sprintf(`INSERT INTO vdb.agent_sessions (session_id, agent_id, parent_session_id, task_id, tool)
...
func provenanceErr(name string, err error) error {
	if strings.Contains(err.Error(), "vdb.agent_sessions") && strings.Contains(err.Error(), "does not exist") {
		return fmt.Errorf("Blackbox provenance isn't installed on %q — run: vdb blackbox upgrade %s", name, name)
	}
	return err
}
...
func AgentSessions(name string, limit int) ([]AgentSession, error) {
  ...
  SELECT a.session_id, a.agent_id, a.parent_session_id, a.task_id, a.tool,
         ...
         (SELECT count(*) FROM vdb.schema_ledger s WHERE s.session = a.session_id) AS entries
  FROM vdb.agent_sessions a ORDER BY a.started_at DESC LIMIT %d) x`, limit))
	if err != nil {
...
func sessionDefaultsSQL(p Provenance) string {
	...
	return set("vdb.session", p.SessionID) + set("vdb.task", p.TaskID) + set("vdb.parent_session", p.ParentSessionID)
}
...
type ChangeNotice struct {
	...
	Policy   *ledger.PolicyDetail `json:"policy,omitempty"` // for SQLSTATE VDB02
}
...
func ExecuteChange(req ExecuteChangeRequest) (ExecuteChangeResult, error) {
	...
	marker := int64(0)
	if l, err := ledgerLines(name, "SELECT coalesce(max(id), 0) FROM vdb.schema_ledger"); err == nil && len(l) > 0 {
		marker, _ = strconv.ParseInt(l[0], 10, 64)
	}

	ctx, cancel := context.WithTimeout(context.Background(), envDurationOr("VECTORADB_EXECUTE_CHANGE_TIMEOUT", 10*time.Minute))
	defer cancel()
	cfg, err := pgx.ParseConfig(fmt.Sprintf("postgres://vdbclient:%s@%s/%s", pgPass(), addr, pgDatabase))
	if err != nil {
		return ExecuteChangeResult{}, err
	}
	...
	if _, err := conn.Exec(ctx, `SELECT set_config('vdb.actor', $1, false), set_config('vdb.actor_kind', 'agent', false),
  set_config('application_name', $2, false), set_config('vdb.session', $3, false), set_config('vdb.task', $4, false),
  set_config('vdb.parent_session', $5, false), set_config('vdb.call_hash', $6, false)`,
	...
	if lines, err := ledgerLines(name, fmt.Sprintf(`SELECT s.id FROM vdb.schema_ledger s LEFT JOIN vdb.ledger_ext e ON e.ledger_id = s.id
