...
// that depend on the table, view or column it changes (vdb.blast_radius,
...
func impactErr(name string, err error) error {
	if strings.Contains(err.Error(), "vdb.blast_radius") && strings.Contains(err.Error(), "does not exist") {
		return fmt.Errorf("Blackbox impact analysis isn't installed on %q — run: vdb blackbox upgrade %s", name, name)
	}
	return err
}
...
func Impact(name, statement, object, column string) (ImpactReport, error) {
	...
	lines, err := ledgerLines(name, fmt.Sprintf("SELECT vdb.blast_radius(%s, %s, 5)::text", quoteLiteral(t.Object), sqlTextOrNull(&t.Column)))
	if err != nil {
...
// parentFromZFSOrigin maps "vectoradb/branches/main@for-qa" to "main".
