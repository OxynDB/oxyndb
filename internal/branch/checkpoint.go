...
// detected. The same checks are available to anyone via cmd/vdb-verify.

// AnchorDir is where a branch's anchor files are written: VECTORADB_ANCHOR_DIR
// (e.g. a write-once mount) or ~/.vectoradb/anchors, plus the branch name.
func AnchorDir(name string) string {
	base := strings.TrimSpace(os.Getenv("VECTORADB_ANCHOR_DIR"))
	if base == "" {
		home, err := os.UserHomeDir()
		if err != nil || home == "" {
			home = "/tmp"
		}
		base = filepath.Join(home, ".vectoradb", "anchors")
	}
...
func ledgerV2Tables(name string) (ext, checkpoints bool, err error) {
	lines, err := ledgerLines(name, "SELECT (to_regclass('vdb.ledger_ext') IS NOT NULL)::text || '|' || "+
		"(to_regclass('vdb.ledger_checkpoints') IS NOT NULL)::text")
	if err != nil {
...
func lastCheckpoint(name string) (int64, string, error) {
	lines, err := ledgerLines(name, "SELECT coalesce((SELECT to_id::text || '|' || merkle_root "+
		"FROM vdb.ledger_checkpoints ORDER BY to_id DESC LIMIT 1), '0|')")
	if err != nil {
		return 0, "", err
	}
	if len(lines) == 0 {
...
func Checkpoint(name string) (*ledger.Anchor, string, error) {
	...
	if !hasCheckpoints {
		return nil, "", fmt.Errorf("ledger checkpoints are not installed on %q — run: vdb ledger upgrade %s", name, name)
	}
	...
	rows, err := loadLedgerRows(name, withExt, fmt.Sprintf(
		"WHERE s.id > %d OR s.id = (SELECT max(id) FROM vdb.schema_ledger WHERE id <= %d AND row_hash IS NOT NULL)",
		lastTo, lastTo))
	if err != nil {
		return nil, "", err
	}
	...
	if err := ledger.CheckChain(pred, fresh); err != nil {
		return nil, "", fmt.Errorf("not anchoring %q: %v — inspect with: vdb ledger integrity %s", name, err, name)
	}
	...
	lines, err := ledgerLines(name, fmt.Sprintf(`INSERT INTO vdb.ledger_checkpoints
  ...
  VALUES (%d, %d, %d, %s, %s, %s, %s, %s) RETURNING id`,
	...
	if truthyEnv("VECTORADB_ANCHOR_IMMUTABLE") {
...
func Integrity(name string) (ledger.Report, error) {
	...
	if hasCheckpoints {
		if lines, err := ledgerLines(name, "SELECT count(*) FROM vdb.ledger_checkpoints"); err == nil && len(lines) > 0 {
		...
		rep.Notes = append(rep.Notes, "ledger checkpoints are not installed on this branch — run: vdb ledger upgrade "+name)
	}
	return rep, nil
}

// ExportLedger writes every ledger row of a branch, with its capture columns, as
// JSON lines — a file vdb-verify can check offline against the anchors.
...
func pendingCheckpointRows(name string) (int, error) {
	...
	lines, err := ledgerLines(name, "SELECT count(*) FROM vdb.schema_ledger WHERE id > "+
		"(SELECT coalesce(max(to_id), 0) FROM vdb.ledger_checkpoints)")
...
// background: once VECTORADB_CHECKPOINT_INTERVAL (default 10m) has passed since
// the branch's last checkpoint, or sooner when VECTORADB_CHECKPOINT_ENTRIES
// (default 500) entries are waiting. It only looks at branches that are already
// running, so it never wakes a suspended one. VECTORADB_CHECKPOINTS=off disables it.
func StartCheckpointer() {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("VECTORADB_CHECKPOINTS"))) {
	case "off", "0", "false", "no":
		log.Printf("ledger checkpoints: scheduler disabled (VECTORADB_CHECKPOINTS)")
		return
	}
	interval := envDurationOr("VECTORADB_CHECKPOINT_INTERVAL", 10*time.Minute)
	threshold := envIntOr("VECTORADB_CHECKPOINT_ENTRIES", 500)
	tick := interval
