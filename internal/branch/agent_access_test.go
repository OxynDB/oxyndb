...
func TestAgentGatewayDSN(t *testing.T) {
	...
	got := agentGatewayDSN("agent-alice", "vdb_secret/key+value")
	u, err := url.Parse(got)
	if err != nil {
	...
	if pw, _ := u.User.Password(); pw != "vdb_secret/key+value" {
...
func TestGatewayHostPort(t *testing.T) {
	...
	// `vdb gateway --addr` is configurable, so agents must be able to be told
	// where it really is.
