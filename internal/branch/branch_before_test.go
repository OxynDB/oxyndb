...
func TestPickBaseBackup(t *testing.T) {
	...
	if !errors.Is(err, ErrNoBaseBackup) || !strings.Contains(err.Error(), "vdb backup create") {
...
func TestEntryQuery(t *testing.T) {
	q := entryQuery(5, true)
	if !strings.Contains(q, "LEFT JOIN vdb.ledger_ext e") || !strings.Contains(q, "WHERE l.id = 5") {
