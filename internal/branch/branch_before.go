...
// vdb.ledger_ext). Entries from before the 2.0 capture existed have no xid, so
...
func entryQuery(entryID int64, withExt bool) string {
	...
	if withExt {
		...
		join = " LEFT JOIN vdb.ledger_ext e ON e.ledger_id = l.id"
	}
  ...
  'xid', %s, 'lsn', %s)
FROM vdb.schema_ledger l%s WHERE l.id = %d`, xid, lsn, join, entryID)
}
...
func pickBaseBackup(bs []walgBackup, systemID string, e beforeEntry) (walgBackup, error) {
	...
	if !found {
		return walgBackup{}, fmt.Errorf("%w: no base backup of this database finished before ledger entry %d (%s) — "+
			"take one with `vdb backup create`; only changes made after a base backup can be branched before",
...
func flushWALArchive(primary string) error {
	...
	deadline := time.Now().Add(envDurationOr("VECTORADB_WAL_ARCHIVE_WAIT", 2*time.Minute))
	for {
...
func waitPromoted(name string, timeout time.Duration) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("the restore did not finish within %s (set VECTORADB_BRANCH_BEFORE_TIMEOUT to wait longer):\n%s",
...
func BranchBeforeEntry(src string, entryID int64, newName string, logf func(format string, a ...any)) (BeforeEntryResult, error) {
	...
	primary := strings.TrimPrefix(PrimaryContainer(), "vec-")
	...
	if err := waitPromoted(newName, envDurationOr("VECTORADB_BRANCH_BEFORE_TIMEOUT", 15*time.Minute)); err != nil {
		return fail(err)
	}

	// 4. The change must not be there.
	present, err := ledgerLines(newName, fmt.Sprintf("SELECT count(*) FROM vdb.schema_ledger WHERE id = %d", entryID))
	if err != nil {
	...
	var last int64
	if l, err := ledgerLines(newName, "SELECT coalesce(max(id), 0) FROM vdb.schema_ledger"); err == nil && len(l) > 0 {
