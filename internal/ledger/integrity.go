...
const AnchorFormat = "vectoradb-ledger-anchor/1"
...
// RowHash recomputes a row's hash exactly as vdb._ledger_hash does in SQL: sha256
...
// link to the previous chained row — the same check `vdb ledger verify` runs in
...
func RowsQuery(withExt bool, where string) string {
	...
	if withExt {
		ext, join = "e.ext_hash", " LEFT JOIN vdb.ledger_ext e ON e.ledger_id = s.id"
	}
	return "SELECT row_to_json(x)::text FROM (SELECT " + rowColumns + ", " + ext +
		" FROM vdb.schema_ledger s" + join + " " + where + " ORDER BY s.id) x"
}

// ExportQuery selects every ledger row with all of its capture columns, one JSON
// document per row in id order — the `vdb ledger export` format.
func ExportQuery(withExt bool) string {
	...
	return "SELECT row_to_json(x)::text FROM (SELECT " + rowColumns +
		...
		" FROM vdb.schema_ledger s LEFT JOIN vdb.ledger_ext e ON e.ledger_id = s.id ORDER BY s.id) x"
}
