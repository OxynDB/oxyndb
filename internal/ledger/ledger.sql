...
--
-- OxynDB Blackbox (RECORD layer) — a record the database keeps about
...
--
-- The ledger lives in schema "odb" (deliberately NOT "oxyndb", which is the
...
DROP EVENT TRIGGER IF EXISTS odb_guard_start;
DROP EVENT TRIGGER IF EXISTS odb_log_end;
DROP EVENT TRIGGER IF EXISTS odb_log_drop;

CREATE EXTENSION IF NOT EXISTS dblink;
CREATE SCHEMA IF NOT EXISTS odb;

CREATE TABLE IF NOT EXISTS odb.schema_ledger (
...
ALTER TABLE odb.schema_ledger ADD COLUMN IF NOT EXISTS prev_hash text;
ALTER TABLE odb.schema_ledger ADD COLUMN IF NOT EXISTS row_hash  text;
CREATE INDEX IF NOT EXISTS schema_ledger_at_idx ON odb.schema_ledger (at DESC);

-- Guardrail policy: op (command tag) -> action.
--   block  refused unless a superuser or odb_admin member sets odb.allow_destructive
--   flag   runs, and its Blackbox entry is recorded FLAGGED
--   allow  runs, recorded as usual (the same as not listing the command)
CREATE TABLE IF NOT EXISTS odb.policy (op text PRIMARY KEY, action text NOT NULL);
INSERT INTO odb.policy(op, action) VALUES
  ('DROP TABLE',  'block'),
  ('DROP SCHEMA', 'block')
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'policy_action_valid'
                 AND conrelid = 'odb.policy'::regclass) THEN
    ALTER TABLE odb.policy ADD CONSTRAINT policy_action_valid
...
-- leave no trace.
CREATE TABLE IF NOT EXISTS odb.policy_history (
  id         bigserial PRIMARY KEY,
  at         timestamptz NOT NULL DEFAULT clock_timestamp(),
  op         text,
...
CREATE OR REPLACE FUNCTION odb.record_policy_change() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, odb AS $$
DECLARE who text := coalesce(nullif(current_setting('odb.actor', true), ''), session_user);
BEGIN
  IF TG_LEVEL = 'STATEMENT' THEN  -- TRUNCATE
    INSERT INTO odb.policy_history (op, change, changed_by) VALUES (NULL, 'truncate', who);
    RETURN NULL;
  END IF;
  INSERT INTO odb.policy_history (op, change, old_action, new_action, changed_by)
...
END;
$$;
CREATE OR REPLACE TRIGGER odb_policy_history AFTER INSERT OR UPDATE OR DELETE ON odb.policy
  FOR EACH ROW EXECUTE FUNCTION odb.record_policy_change();
CREATE OR REPLACE TRIGGER odb_policy_history_truncate AFTER TRUNCATE ON odb.policy
  FOR EACH STATEMENT EXECUTE FUNCTION odb.record_policy_change();
CREATE OR REPLACE FUNCTION odb.deny_history_change() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'odb.% is append-only — its history cannot be modified', TG_TABLE_NAME;
END;
$$;
CREATE OR REPLACE TRIGGER odb_policy_history_append_only BEFORE UPDATE OR DELETE ON odb.policy_history
  FOR EACH ROW EXECUTE FUNCTION odb.deny_history_change();
CREATE OR REPLACE TRIGGER odb_policy_history_no_truncate BEFORE TRUNCATE ON odb.policy_history
  FOR EACH STATEMENT EXECUTE FUNCTION odb.deny_history_change();
REVOKE INSERT, UPDATE, DELETE, TRUNCATE ON odb.policy_history FROM PUBLIC;

-- Who may override a blocking policy with SET odb.allow_destructive=on: superusers
-- (the engine's own imports and pipelines connect as one) and members of
-- odb_admin. Any other session stays blocked even with the override set. Membership
-- is granted per user with `odb admin grant <email>`.
DO $$
BEGIN
...
CREATE OR REPLACE FUNCTION odb._may_override() RETURNS boolean
LANGUAGE sql STABLE AS $$
  SELECT coalesce((SELECT rolsuper FROM pg_roles WHERE rolname = session_user), false)
      OR pg_has_role(session_user, 'odb_admin', 'MEMBER');
$$;
...
-- the connection: odb.actor_kind is an ordinary session setting, so a client
-- could `SET odb.actor_kind = 'human'` and have its changes recorded as a
...
-- agent framework naming its own session is the point of odb.session. They
-- describe the change; actor and actor_kind attribute it.
CREATE OR REPLACE FUNCTION odb._ctx(
  OUT actor text, OUT actor_kind text, OUT tool text, OUT session text, OUT branch text
) LANGUAGE sql STABLE AS $$
  SELECT CASE WHEN session_user NOT IN ('oxyndb','odbclient')
              THEN session_user
              ELSE current_setting('odb.actor', true) END,
         CASE
           ...
           WHEN current_setting('odb.actor_kind', true) = 'agent' THEN 'agent'
           ELSE 'human'
         END,
         nullif(current_setting('application_name', true), ''),
         coalesce(nullif(current_setting('odb.session', true), ''), pg_backend_pid()::text),
         current_setting('odb.branch', true);
$$;

-- Should this DDL be recorded, or is it internal noise?
CREATE OR REPLACE FUNCTION odb._skip(schema_name text, command_tag text)
RETURNS boolean LANGUAGE sql IMMUTABLE AS $$
  ...
  SELECT schema_name = 'odb'   -- the ledger's own objects
...
CREATE OR REPLACE FUNCTION odb._statement_words(body text) RETURNS text[]
...
CREATE OR REPLACE FUNCTION odb._statement_has_tag(body text, tag text) RETURNS boolean
LANGUAGE plpgsql IMMUTABLE AS $$
DECLARE
  w  text[] := odb._statement_words(body);
...
CREATE OR REPLACE FUNCTION odb._statement_fires_ddl(body text) RETURNS boolean
LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE
    ...
    ELSE false
  END
  FROM (SELECT odb._statement_words(body) AS w) x;
$$;
...
CREATE OR REPLACE FUNCTION odb._statement_candidates(q text, tag text) RETURNS text[]
LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE WHEN cardinality(m) > 0 THEN m ELSE ARRAY[q] END
  FROM (SELECT ARRAY(SELECT s.body FROM odb._sql_statements(q) s
                     WHERE odb._statement_has_tag(s.body, tag) ORDER BY s.pos) AS m) x;
$$;
...
CREATE OR REPLACE FUNCTION odb._statement_texts(p_event text, p_tag text, p_ctx text) RETURNS text[]
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, odb AS $$
DECLARE
  q      text := current_query();
  me     int  := pg_backend_pid();
  key    text;
  cur    odb.statement_cursor;
  nested text;
  st     int;
  k      int;
  e      int;
BEGIN
  IF strpos(coalesce(p_ctx, ''), E'\n') > 0 THEN
    ...
    END IF;
    RETURN odb._statement_candidates(nested, p_tag) || ARRAY[nested, q];
  END IF;

  key := md5(q) || '@' || statement_timestamp()::text;
  SELECT * INTO cur FROM odb.statement_cursor WHERE pid = me AND event = p_event;
  IF NOT FOUND OR cur.run_key <> key THEN
    cur := ROW(me, p_event, key, ARRAY(SELECT s.body FROM odb._sql_statements(q) s ORDER BY s.pos), 0, true);
    cur.in_step := cardinality(cur.statements) > 0;
    INSERT INTO odb.statement_cursor VALUES (cur.*)
  ...
  FOR k IN cur.pos + 1 .. cardinality(cur.statements) LOOP
    IF odb._statement_has_tag(cur.statements[k], p_tag) THEN
      UPDATE odb.statement_cursor SET pos = k WHERE pid = me AND event = p_event;
      RETURN ARRAY[cur.statements[k]];
    END IF;
    EXIT WHEN odb._statement_fires_ddl(cur.statements[k]);
  END LOOP;
  UPDATE odb.statement_cursor SET in_step = false WHERE pid = me AND event = p_event;
  RETURN ARRAY[q];
END;
$$;

-- Does this session already hold the lock that serializes Blackbox appends
-- (odb.chain_row)? A blocked attempt is recorded through a second connection
...
CREATE OR REPLACE FUNCTION odb._holds_chain_lock() RETURNS boolean
LANGUAGE sql STABLE AS $$
  SELECT EXISTS (
    SELECT 1 FROM pg_locks
    WHERE locktype = 'advisory' AND pid = pg_backend_pid() AND granted AND objsubid = 1
      AND ((classid::bigint << 32) | objid::bigint) = hashtext('odb.schema_ledger')::bigint);
$$;
...
CREATE OR REPLACE FUNCTION odb.guard_ddl_start() RETURNS event_trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, odb AS $$
DECLARE act text; c record; allow text;
BEGIN
  SELECT action INTO act FROM odb.policy WHERE op = TG_TAG;
  IF act IS DISTINCT FROM 'block' THEN
    RETURN; -- allowed (or flagged) — recorded in the end/drop triggers
  END IF;
  allow := coalesce(nullif(current_setting('odb.allow_destructive', true), ''), 'off');
  IF allow IN ('on','true','1') AND odb._may_override() THEN
    RETURN; -- approved override (a superuser or a odb_admin member)
  END IF;
  SELECT * INTO c FROM odb._ctx();
  -- Record the blocked attempt durably via dblink (autonomous — survives the rollback),
  -- unless that would deadlock with this transaction (odb._holds_chain_lock).
  IF odb._holds_chain_lock() THEN
    RAISE WARNING 'OxynDB guardrail: this blocked attempt is not recorded in Blackbox, because this transaction has already written to it';
  ELSE
    PERFORM dblink_exec(
      'host=/var/run/postgresql dbname=' || current_database() || ' user=' || current_user,
      format($f$INSERT INTO odb.schema_ledger
  ...
  END IF;
  RAISE EXCEPTION 'OxynDB guardrail: % is blocked by policy (set odb.allow_destructive=on to override)', TG_TAG
...
CREATE OR REPLACE FUNCTION odb.log_ddl_end() RETURNS event_trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, odb AS $$
...
BEGIN
  ...
  -- odb._statement_texts); if that can't be established, on the whole query.
  BEGIN
  ...
  FOR r IN SELECT * FROM pg_event_trigger_ddl_commands() LOOP
    ...
    IF odb._skip(r.schema_name, r.command_tag) THEN CONTINUE; END IF;
    ...
    IF st = 'APPLIED' AND EXISTS (SELECT 1 FROM odb.policy WHERE op = r.command_tag AND action = 'flag') THEN
      st := 'FLAGGED'; rk := 'policy';
    END IF;
    INSERT INTO odb.schema_ledger
...
CREATE OR REPLACE FUNCTION odb.log_ddl_drop() RETURNS event_trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, odb AS $$
DECLARE r record; c record; q text; st text := 'APPLIED';
BEGIN
  SELECT * INTO c FROM odb._ctx();
  q := current_query();
  IF EXISTS (SELECT 1 FROM odb.policy WHERE op = TG_TAG AND action = 'flag') THEN
  ...
  FOR r IN SELECT * FROM pg_event_trigger_dropped_objects() WHERE NOT is_temporary AND original LOOP
    IF odb._skip(r.schema_name, TG_TAG) THEN CONTINUE; END IF;
...
-- `odb ledger verify` recomputes the chain to catch it. Detection holds no matter
...
CREATE OR REPLACE FUNCTION odb._ledger_hash(r odb.schema_ledger) RETURNS text
LANGUAGE sql IMMUTABLE AS $$
...
CREATE OR REPLACE FUNCTION odb.chain_row() RETURNS trigger
LANGUAGE plpgsql AS $$
DECLARE prev text;
BEGIN
  -- Serialize appends so two concurrent inserts can't chain off the same row.
  PERFORM pg_advisory_xact_lock(hashtext('odb.schema_ledger'));
  SELECT row_hash INTO prev FROM odb.schema_ledger ORDER BY id DESC LIMIT 1;
  NEW.prev_hash := coalesce(prev, '');
  NEW.row_hash  := odb._ledger_hash(NEW);
  RETURN NEW;
END;
$$;
CREATE OR REPLACE TRIGGER odb_chain BEFORE INSERT ON odb.schema_ledger
  FOR EACH ROW EXECUTE FUNCTION odb.chain_row();

-- Append-only: the ledger records history, so history cannot be rewritten.
CREATE OR REPLACE FUNCTION odb.deny_change() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'odb.schema_ledger is append-only — its history cannot be modified';
END;
$$;
CREATE OR REPLACE TRIGGER odb_append_only BEFORE UPDATE OR DELETE ON odb.schema_ledger
  FOR EACH ROW EXECUTE FUNCTION odb.deny_change();
CREATE OR REPLACE TRIGGER odb_no_truncate BEFORE TRUNCATE ON odb.schema_ledger
  FOR EACH STATEMENT EXECUTE FUNCTION odb.deny_change();
REVOKE UPDATE, DELETE, TRUNCATE ON odb.schema_ledger FROM PUBLIC;

CREATE EVENT TRIGGER odb_guard_start ON ddl_command_start
  EXECUTE FUNCTION odb.guard_ddl_start();
CREATE EVENT TRIGGER odb_log_end ON ddl_command_end
  EXECUTE FUNCTION odb.log_ddl_end();
CREATE EVENT TRIGGER odb_log_drop ON sql_drop
  EXECUTE FUNCTION odb.log_ddl_drop();
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'odbclient') THEN
    CREATE ROLE odbclient LOGIN NOSUPERUSER NOCREATEROLE NOCREATEDB NOBYPASSRLS;
  END IF;
END $$;
...
GRANT USAGE, CREATE ON SCHEMA public TO odbclient;
GRANT ALL ON ALL TABLES IN SCHEMA public TO odbclient;
GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO odbclient;
ALTER DEFAULT PRIVILEGES FOR ROLE oxyndb IN SCHEMA public GRANT ALL ON TABLES TO odbclient;
ALTER DEFAULT PRIVILEGES FOR ROLE oxyndb IN SCHEMA public GRANT ALL ON SEQUENCES TO odbclient;
...
GRANT USAGE ON SCHEMA odb TO odbclient;
GRANT SELECT ON odb.schema_ledger, odb.policy, odb.policy_history TO odbclient;
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA odb TO odbclient;
...
REVOKE EXECUTE ON FUNCTION odb._statement_texts(text, text, text) FROM PUBLIC, odbclient;
