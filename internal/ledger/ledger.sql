...
--   block  refused unless a superuser or vdb_admin member sets vdb.allow_destructive
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'policy_action_valid'
                 AND conrelid = 'vdb.policy'::regclass) THEN
    ALTER TABLE vdb.policy ADD CONSTRAINT policy_action_valid
...
-- leave no trace.
CREATE TABLE IF NOT EXISTS vdb.policy_history (
  id         bigserial PRIMARY KEY,
  at         timestamptz NOT NULL DEFAULT clock_timestamp(),
  op         text,
...
CREATE OR REPLACE FUNCTION vdb.record_policy_change() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, vdb AS $$
DECLARE who text := coalesce(nullif(current_setting('vdb.actor', true), ''), session_user);
BEGIN
  IF TG_LEVEL = 'STATEMENT' THEN  -- TRUNCATE
    INSERT INTO vdb.policy_history (op, change, changed_by) VALUES (NULL, 'truncate', who);
    RETURN NULL;
  END IF;
  INSERT INTO vdb.policy_history (op, change, old_action, new_action, changed_by)
...
END;
$$;
CREATE OR REPLACE TRIGGER vdb_policy_history AFTER INSERT OR UPDATE OR DELETE ON vdb.policy
  FOR EACH ROW EXECUTE FUNCTION vdb.record_policy_change();
CREATE OR REPLACE TRIGGER vdb_policy_history_truncate AFTER TRUNCATE ON vdb.policy
  FOR EACH STATEMENT EXECUTE FUNCTION vdb.record_policy_change();
CREATE OR REPLACE FUNCTION vdb.deny_history_change() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'vdb.% is append-only — its history cannot be modified', TG_TABLE_NAME;
END;
$$;
CREATE OR REPLACE TRIGGER vdb_policy_history_append_only BEFORE UPDATE OR DELETE ON vdb.policy_history
  FOR EACH ROW EXECUTE FUNCTION vdb.deny_history_change();
CREATE OR REPLACE TRIGGER vdb_policy_history_no_truncate BEFORE TRUNCATE ON vdb.policy_history
  FOR EACH STATEMENT EXECUTE FUNCTION vdb.deny_history_change();
REVOKE INSERT, UPDATE, DELETE, TRUNCATE ON vdb.policy_history FROM PUBLIC;
...
CREATE OR REPLACE FUNCTION vdb._sql_statements(q text)
...
CREATE OR REPLACE FUNCTION vdb._statement_words(body text) RETURNS text[]
...
CREATE OR REPLACE FUNCTION vdb._statement_has_tag(body text, tag text) RETURNS boolean
LANGUAGE plpgsql IMMUTABLE AS $$
DECLARE
  w  text[] := vdb._statement_words(body);
...
CREATE OR REPLACE FUNCTION vdb._statement_fires_ddl(body text) RETURNS boolean
LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE
    ...
    ELSE false
  END
  FROM (SELECT vdb._statement_words(body) AS w) x;
$$;
...
CREATE OR REPLACE FUNCTION vdb._statement_candidates(q text, tag text) RETURNS text[]
LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE WHEN cardinality(m) > 0 THEN m ELSE ARRAY[q] END
  FROM (SELECT ARRAY(SELECT s.body FROM vdb._sql_statements(q) s
                     WHERE vdb._statement_has_tag(s.body, tag) ORDER BY s.pos) AS m) x;
$$;
...
CREATE UNLOGGED TABLE IF NOT EXISTS vdb.statement_cursor (
  pid        int     NOT NULL,
  event      text    NOT NULL,
...
REVOKE ALL ON vdb.statement_cursor FROM PUBLIC;
...
CREATE OR REPLACE FUNCTION vdb._statement_texts(p_event text, p_tag text, p_ctx text) RETURNS text[]
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, vdb AS $$
DECLARE
  q      text := current_query();
  me     int  := pg_backend_pid();
  key    text;
  cur    vdb.statement_cursor;
  nested text;
  st     int;
  k      int;
  e      int;
BEGIN
  IF strpos(coalesce(p_ctx, ''), E'\n') > 0 THEN
    ...
    END IF;
    RETURN vdb._statement_candidates(nested, p_tag) || ARRAY[nested, q];
  END IF;

  key := md5(q) || '@' || statement_timestamp()::text;
  SELECT * INTO cur FROM vdb.statement_cursor WHERE pid = me AND event = p_event;
  IF NOT FOUND OR cur.run_key <> key THEN
    cur := ROW(me, p_event, key, ARRAY(SELECT s.body FROM vdb._sql_statements(q) s ORDER BY s.pos), 0, true);
    cur.in_step := cardinality(cur.statements) > 0;
    INSERT INTO vdb.statement_cursor VALUES (cur.*)
  ...
  FOR k IN cur.pos + 1 .. cardinality(cur.statements) LOOP
    IF vdb._statement_has_tag(cur.statements[k], p_tag) THEN
      UPDATE vdb.statement_cursor SET pos = k WHERE pid = me AND event = p_event;
      RETURN ARRAY[cur.statements[k]];
    END IF;
    EXIT WHEN vdb._statement_fires_ddl(cur.statements[k]);
  END LOOP;
  UPDATE vdb.statement_cursor SET in_step = false WHERE pid = me AND event = p_event;
  RETURN ARRAY[q];
END;
$$;

-- Does this session already hold the lock that serializes Blackbox appends
-- (vdb.chain_row)? A blocked attempt is recorded through a second connection
...
CREATE OR REPLACE FUNCTION vdb._holds_chain_lock() RETURNS boolean
LANGUAGE sql STABLE AS $$
  SELECT EXISTS (
    SELECT 1 FROM pg_locks
    WHERE locktype = 'advisory' AND pid = pg_backend_pid() AND granted AND objsubid = 1
      AND ((classid::bigint << 32) | objid::bigint) = hashtext('vdb.schema_ledger')::bigint);
$$;
...
BEGIN
  ...
  -- unless that would deadlock with this transaction (vdb._holds_chain_lock).
  IF vdb._holds_chain_lock() THEN
    RAISE WARNING 'VectoraDB guardrail: this blocked attempt is not recorded in Blackbox, because this transaction has already written to it';
  ELSE
    PERFORM dblink_exec(
...
BEGIN
  ...
  -- vdb._statement_texts); if that can't be established, on the whole query.
  BEGIN
    GET DIAGNOSTICS ctx = PG_CONTEXT;
    t := vdb._statement_texts('end', TG_TAG, ctx);
  ...
  FOR r IN SELECT * FROM pg_event_trigger_ddl_commands() LOOP
    ...
    IF st = 'APPLIED' AND EXISTS (SELECT 1 FROM vdb.policy WHERE op = r.command_tag AND action = 'flag') THEN
...
BEGIN
  SELECT * INTO c FROM vdb._ctx();
  q := current_query();
  IF EXISTS (SELECT 1 FROM vdb.policy WHERE op = TG_TAG AND action = 'flag') THEN
...
GRANT SELECT ON vdb.schema_ledger, vdb.policy, vdb.policy_history TO vdbclient;
...
REVOKE EXECUTE ON FUNCTION vdb._statement_texts(text, text, text) FROM PUBLIC, vdbclient;
