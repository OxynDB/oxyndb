...
func TestSchemaProvenance(t *testing.T) {
	...
	for _, want := range []string{
		...
		"BEFORE UPDATE OR DELETE ON vdb.agent_sessions", // append-only
		"BEFORE TRUNCATE ON vdb.agent_sessions",
	...
	for _, re := range []*regexp.Regexp{
		regexp.MustCompile(`(?i)alter\s+table\s+(if\s+exists\s+)?vdb\.(schema_ledger|ledger_ext|policy)\b`),
		regexp.MustCompile(`(?i)drop\s+(table|trigger|function|event\s+trigger|index)`),
		regexp.MustCompile(`(?i)create\s+(or\s+replace\s+)?function\s+vdb\.(capture_ext|deny_ext_change|_ctx|_ledger_hash|chain_row)\b`),
		regexp.MustCompile(`(?i)on\s+vdb\.schema_ledger\b`),
	} {
