...
--
-- VectoraDB Blackbox policy gate (Blackbox 2.0 Phase 5) — installed AFTER
-- ledger.sql and ledger_v2.sql. What clients receive is a public contract:
-- docs/policy-errors.md (SQLSTATE VDB01 block / VDB02 warn, JSON DETAIL).
--
-- Rules are checked on ddl_command_start by the event trigger vdb_policy_start,
-- which fires after the existing guardrail vdb_guard_start (event triggers fire
-- in name order). Nothing here changes the guardrail, vdb.policy or the ledger.
--
-- Safety rules for this file:
--   * Fail-safe: any error while matching or recording becomes a WARNING and the
--     statement runs. The one deliberate error is a policy block (VDB01).
--   * Every shipped rule warns; blocking is opt-in per rule.
--   * The gate does NOT honour the session setting vdb.v2 (a client could set
--     it to skip a block). A superuser turns the gate off with:
--       ALTER EVENT TRIGGER vdb_policy_start DISABLE;
...
CREATE OR REPLACE FUNCTION vdb._valid_regex(p text) RETURNS boolean
...
INSERT INTO vdb.policy_rules (rule_id, command_tag, pattern, action, reason, hint, builtin) VALUES
  ('alter-column-type', 'ALTER TABLE', '\malter\M[^;]*\mtype\M', 'warn',
   ...
   'Try it on a branch first: vdb branch create try-it', true),
  ('drop-column', 'ALTER TABLE',
   ...
   'Check what uses the column first, or try it on a branch: vdb branch create try-it', true),
  ('drop-index', 'DROP INDEX', NULL, 'warn',
   'dropping an index can make the queries that use it much slower',
   'Check the query plans that use it first, or try it on a branch: vdb branch create try-it', true),
...
REVOKE INSERT, UPDATE, DELETE, TRUNCATE ON vdb.policy_rules FROM PUBLIC;
GRANT SELECT ON vdb.policy_rules TO vdbclient;

-- Every change to a rule, append-only. Rules are changed through the engine
-- (vdb policy …, REST), which sets vdb.actor for the transaction.
CREATE TABLE IF NOT EXISTS vdb.policy_rule_history (
...
CREATE OR REPLACE FUNCTION vdb.record_policy_rule_change() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, vdb AS $$
BEGIN
  BEGIN
    INSERT INTO vdb.policy_rule_history (rule_id, change, old_rule, new_rule, changed_by)
    VALUES (coalesce(NEW.rule_id, OLD.rule_id), lower(TG_OP),
            CASE WHEN TG_OP <> 'INSERT' THEN to_jsonb(OLD) END,
            CASE WHEN TG_OP <> 'DELETE' THEN to_jsonb(NEW) END,
            coalesce(nullif(current_setting('vdb.actor', true), ''), session_user));
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'VectoraDB Blackbox policy: rule change not recorded: %', SQLERRM;
  END;
  RETURN NULL;
END;
$$;
CREATE OR REPLACE TRIGGER vdb_policy_rule_history AFTER INSERT OR UPDATE OR DELETE ON vdb.policy_rules
  FOR EACH ROW EXECUTE FUNCTION vdb.record_policy_rule_change();
CREATE OR REPLACE TRIGGER vdb_policy_history_append_only BEFORE UPDATE OR DELETE ON vdb.policy_rule_history
  FOR EACH ROW EXECUTE FUNCTION vdb.deny_ext_change();
CREATE OR REPLACE TRIGGER vdb_policy_history_no_truncate BEFORE TRUNCATE ON vdb.policy_rule_history
  FOR EACH STATEMENT EXECUTE FUNCTION vdb.deny_ext_change();
REVOKE INSERT, UPDATE, DELETE, TRUNCATE ON vdb.policy_rule_history FROM PUBLIC;
GRANT SELECT ON vdb.policy_rule_history TO vdbclient;

-- ── Evaluations: every warning, block and override ─────────────────────────
CREATE TABLE IF NOT EXISTS vdb.ledger_policy_evaluations (
  id            bigserial PRIMARY KEY,
  at            timestamptz NOT NULL DEFAULT clock_timestamp(),
  xid           bigint,        -- the statement's transaction (joins vdb.ledger_ext.xid)
  rule_id       text NOT NULL,
...
CREATE INDEX IF NOT EXISTS ledger_policy_evaluations_xid_idx ON vdb.ledger_policy_evaluations (xid);
CREATE OR REPLACE TRIGGER vdb_policy_eval_append_only BEFORE UPDATE OR DELETE ON vdb.ledger_policy_evaluations
  FOR EACH ROW EXECUTE FUNCTION vdb.deny_ext_change();
CREATE OR REPLACE TRIGGER vdb_policy_eval_no_truncate BEFORE TRUNCATE ON vdb.ledger_policy_evaluations
  FOR EACH STATEMENT EXECUTE FUNCTION vdb.deny_ext_change();
REVOKE INSERT, UPDATE, DELETE, TRUNCATE ON vdb.ledger_policy_evaluations FROM PUBLIC;
GRANT SELECT ON vdb.ledger_policy_evaluations TO vdbclient;

-- ── The contract (docs/policy-errors.md) ────────────────────────────────────
CREATE OR REPLACE FUNCTION vdb._policy_hint(r vdb.policy_rules) RETURNS text
LANGUAGE sql STABLE AS $$
  SELECT coalesce(nullif(r.hint, ''), 'Try the change on a branch first: vdb branch create try-it');
$$;

CREATE OR REPLACE FUNCTION vdb._policy_detail(r vdb.policy_rules, command text, evaluation_id bigint, blackbox_id bigint)
RETURNS jsonb LANGUAGE sql STABLE AS $$
  SELECT jsonb_build_object(
    ...
    'matched', r.pattern,
    'reason', r.reason,
    'hint', vdb._policy_hint(r),
    'override', CASE WHEN r.action = 'block' THEN 'vdb_admin' END,
...
-- ── The gate ────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION vdb.policy_ddl_start() RETURNS event_trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public, vdb AS $$
DECLARE
  q        text;
  r        vdb.policy_rules;
...
BEGIN
  ...
  BEGIN
    ...
    FOR r IN SELECT * FROM vdb.policy_rules
      ...
      IF r.action = 'block' THEN
        IF may IS NULL THEN
          may := vdb._may_override();
        END IF;
        IF may AND r.rule_id = ANY (allow) THEN
          -- An approved override: record it and let the statement through.
          INSERT INTO vdb.ledger_policy_evaluations (xid, rule_id, action, command_tag, statement_md5, actor, session)
          VALUES (txid_current(), r.rule_id, 'allowed', TG_TAG, md5(q), c.actor, c.session);
          PERFORM set_config('vdb.policy_override_used', 'on', true);
          CONTINUE;
        END IF;
        blk := r;
        blocking := true;
      ...
      RAISE NOTICE USING ERRCODE = 'VDB02',
        MESSAGE = format('Blackbox policy warning: %s (rule %s)', r.reason, r.rule_id),
        DETAIL  = vdb._policy_detail(r, TG_TAG, eval_id, NULL)::text,
        HINT    = vdb._policy_hint(r);
    END LOOP;
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'VectoraDB Blackbox policy: evaluation skipped: %', SQLERRM;
    RETURN;
  END;
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'VectoraDB Blackbox policy: could not record the blocked attempt: %', SQLERRM;
  END;

  -- 3. Refuse the statement.
  RAISE EXCEPTION USING ERRCODE = 'VDB01',
    MESSAGE = format('Blackbox policy: %s (rule %s)', blk.reason, blk.rule_id),
    DETAIL  = vdb._policy_detail(blk, TG_TAG, eval_id, bb_id)::text,
    HINT    = vdb._policy_hint(blk)
              || format(' — or ask a Blackbox admin to allow this rule for the session: SET vdb.policy_allow = %L', blk.rule_id);
END;
$$;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_event_trigger WHERE evtname = 'vdb_policy_start') THEN
    CREATE EVENT TRIGGER vdb_policy_start ON ddl_command_start
      EXECUTE FUNCTION vdb.policy_ddl_start();
  END IF;
END $$;
