...
// The same fixed row the integration test recomputes in SQL (vdb._ledger_hash).
...
func TestQueries(t *testing.T) {
	...
	if q := RowsQuery(true, "WHERE s.id > 5"); !strings.Contains(q, "LEFT JOIN vdb.ledger_ext") || !strings.Contains(q, "WHERE s.id > 5 ORDER BY s.id") {
