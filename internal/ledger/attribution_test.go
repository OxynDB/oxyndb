...
// vdb.actor_kind is an ordinary session setting, so while _ctx believed it a
// client could `SET vdb.actor_kind = 'human'` and have an agent's changes
...
func TestActorKindDerivedFromRole(t *testing.T) {
	...
	kindLine := regexp.MustCompile(`(?s)OUT actor_kind.*?current_setting\('vdb\.actor_kind'`)
	if !kindLine.MatchString(ctx) {
		t.Fatal("_ctx does not mention vdb.actor_kind at all; expected it as the shared-role fallback")
	}
	before, _, _ := strings.Cut(ctx, "current_setting('vdb.actor_kind'")
	if !strings.Contains(before, "session_user LIKE 'agent-%'") {
		t.Error("the session setting is consulted before the role is examined")
	}
}

// ctxFunction returns the body of vdb._ctx from the embedded schema.
func ctxFunction(t *testing.T) string {
	t.Helper()
	const marker = "CREATE OR REPLACE FUNCTION vdb._ctx("
	i := strings.Index(Schema, marker)
	if i < 0 {
		t.Fatal("vdb._ctx is not in the schema")
	}
	rest := Schema[i:]
	end := strings.Index(rest, "$$;")
	if end < 0 {
		t.Fatal("vdb._ctx has no end")
	}
	return rest[:end]
}
