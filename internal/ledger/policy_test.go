...
func TestSchemaPolicy(t *testing.T) {
	...
	for _, want := range []string{
		...
		"vdb._may_override()",              // same override check as the guardrail
		...
		"EXCEPTION WHEN OTHERS THEN\n    RAISE WARNING 'VectoraDB Blackbox policy: evaluation skipped",
	...
	if strings.Contains(SchemaPolicy, "current_setting('vdb.v2'") {
		t.Error("the policy gate must not honour the session setting vdb.v2")
	}
	forbidden := []*regexp.Regexp{
		regexp.MustCompile(`(?i)alter\s+table\s+(if\s+exists\s+)?vdb\.(schema_ledger|policy)\b`),
		...
		regexp.MustCompile(`(?i)create\s+(or\s+replace\s+)?function\s+vdb\.(_ledger_hash|chain_row|deny_change|guard_ddl_start|log_ddl_end|log_ddl_drop|_ctx|_skip|_may_override|capture_ext|deny_ext_change)\b`),
...
func TestParsePolicyDetail(t *testing.T) {
	d, err := ParsePolicyDetail(`{"v": 1, "hint": "h", "action": "block", "impact": null, "reason": "r", "command": "ALTER TABLE",
		"matched": "x", "rule_id": "drop-column", "override": "vdb_admin", "blackbox_id": 918, "evaluation_id": 42, "future_key": true}`)
	if err != nil {
		t.Fatal(err)
	}
	if d.RuleID != "drop-column" || d.Action != "block" || *d.BlackboxID != 918 || *d.EvaluationID != 42 || *d.Override != "vdb_admin" {
		t.Fatalf("parsed %+v", d)
	}
