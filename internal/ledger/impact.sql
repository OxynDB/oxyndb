...
--
-- VectoraDB Blackbox impact analysis (Blackbox 2.0 Phase 7) — installed AFTER
-- the other Blackbox objects.
--
-- vdb.blast_radius reports what depends on a table, view, materialized view,
...
CREATE OR REPLACE FUNCTION vdb.blast_radius(target text, target_column text DEFAULT NULL, max_depth integer DEFAULT 5)
RETURNS jsonb
...
END;
$$;
GRANT EXECUTE ON FUNCTION vdb.blast_radius(text, text, integer) TO vdbclient;

-- Which impact-analysis definition is installed.
CREATE OR REPLACE FUNCTION vdb.blackbox_impact_version() RETURNS text
LANGUAGE sql IMMUTABLE AS $$ SELECT '1' $$;
GRANT EXECUTE ON FUNCTION vdb.blackbox_impact_version() TO vdbclient;
