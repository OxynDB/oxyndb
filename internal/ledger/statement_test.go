...
func TestSchemaStatementMatching(t *testing.T) {
	...
	for _, want := range []string{
		...
		"CREATE OR REPLACE FUNCTION vdb._statement_texts(p_event text, p_tag text, p_ctx text)",
		...
		"'VectoraDB guardrail: % is blocked by policy (set vdb.allow_destructive=on to override)'",
	} {
	...
	grant := strings.Index(ledgerSQL, "GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA vdb TO vdbclient;")
	revoke := strings.Index(ledgerSQL, "REVOKE EXECUTE ON FUNCTION vdb._statement_texts(text, text, text) FROM PUBLIC, vdbclient;")
	...
	if regexp.MustCompile(`(?i)grant[^;]*vdb\.statement_cursor[^;]*to`).MatchString(ledgerSQL + policySQL) {
		t.Error("clients must not be granted anything on vdb.statement_cursor")
	}
	...
	if strings.Contains(policySQL, "q ~* pattern") || !strings.Contains(policySQL, "texts := vdb._statement_texts('start', TG_TAG, ctx);") {
		t.Error("the policy gate must match rules against vdb._statement_texts, not the whole query")
	}
	if !strings.Contains(policySQL, "unnest(vdb._statement_candidates(statement, command))") {
	...
	for name, sql := range map[string]string{"ledger.sql (guardrail)": ledgerSQL, "policy.sql (gate)": policySQL} {
		lock := strings.Index(sql, "IF vdb._holds_chain_lock() THEN")
		write := strings.Index(sql, "INSERT INTO vdb.schema_ledger")
		if lock < 0 || write < 0 {
	...
	for _, want := range []string{"action = 'flag'", "CHECK (action IN ('block','flag','allow')) NOT VALID", "CREATE TABLE IF NOT EXISTS vdb.policy_history"} {
		if !strings.Contains(ledgerSQL, want) {
			t.Errorf("ledger.sql is missing %q", want)
		}
	}
