...
func TestRequiredAssets(t *testing.T) {
	for _, c := range []struct {
		...
		{Target{GOOS: "darwin", HostArch: "arm64", GuestArch: "arm64"}, "vdb-darwin-arm64 vdb-linux-arm64"},
		{Target{GOOS: "darwin", HostArch: "amd64"}, "vdb-darwin-amd64 vdb-linux-amd64"},
		{Target{GOOS: "windows", HostArch: "amd64"}, "vdb-windows-amd64.exe vdb-linux-amd64 vectoradb-docker-context.tar.gz"},
		{Target{GOOS: "linux", HostArch: "arm64"}, "vdb-linux-arm64"},
	} {
...
func TestCandidates(t *testing.T) {
	...
	full := []string{"SHA256SUMS", "vdb-linux-amd64", "vdb-darwin-arm64"}
	rels := []Release{
	...
	// darwin needs vdb-linux-arm64, which none of these have.
...
func TestChecksums(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "vdb-linux-amd64")
	os.WriteFile(path, []byte("binary"), 0o755)
	sum := sha256.Sum256([]byte("binary"))
	hexsum := hex.EncodeToString(sum[:])
	text := strings.ToUpper(hexsum) + " *vdb-linux-amd64\r\n\r\n# comment\n" + strings.Repeat("a", 64) + "  other file.tar.gz\n"
	sums, err := ParseChecksums(strings.NewReader(text))
	if err != nil {
		t.Fatal(err)
	}
	if sums["other file.tar.gz"] == "" || sums["vdb-linux-amd64"] != hexsum {
		t.Fatalf("parsed %v", sums)
	}
	if err := sums.Verify("vdb-linux-amd64", path); err != nil {
		t.Fatalf("verify: %v", err)
	}
	os.WriteFile(path, []byte("tampered"), 0o755)
	if err := sums.Verify("vdb-linux-amd64", path); err == nil || !strings.Contains(err.Error(), "mismatch") {
...
func TestResolveAndDownload(t *testing.T) {
	...
	f.publish("v0.99.1", map[string]string{"vdb-linux-amd64": "unlisted"}, "vdb-linux-amd64") // SHA256SUMS doesn't list it
	...
	if n := Notice("0.98.0", o); n != "VectoraDB v0.99.0 is available (you have 0.98.0). Run `vdb update` to get the new capabilities." {
		t.Fatalf("notice %q", n)
	}
	...
	dir := t.TempDir()
	a, _ := o.Release.Asset("vdb-linux-amd64")
	path, err := c.Download(ctx, a, o.Sums["vdb-linux-amd64"], dir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestShouldCheck(t *testing.T) {
	...
	env[EnvNoCheck] = "1"
	if ShouldCheck(get, "0.8.2") {
		t.Error("VECTORADB_NO_UPDATE_CHECK=1 must turn the check off")
	}
	env["VECTORADB_UPDATE_CHECK_TIMEOUT"] = "3s"
...
func TestInstallBinary(t *testing.T) {
	...
	dest := filepath.Join(dir, "bin", "vdb")
	os.MkdirAll(filepath.Dir(dest), 0o755)
	os.WriteFile(src, []byte("v2"), 0o644)
	os.WriteFile(dest, []byte("v1"), 0o755)
	prev := filepath.Join(dir, "updates", "prev", "vdb")
	...
	if _, err := os.Stat(filepath.Join(dir, "bin", ".vdb.new")); err == nil {
...
func TestSwapExecutable(t *testing.T) {
	...
	dest := filepath.Join(dir, "vdb.exe")
	prev := filepath.Join(dir, "prev", "vdb.exe")
	os.WriteFile(src, []byte("v2"), 0o644)
	os.WriteFile(dest, []byte("v1"), 0o755)
...
// The release workflow must publish every file `vdb update` needs, for every
