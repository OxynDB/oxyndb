...
// UpdateOptions are the flags of `vdb update`.
...
type updater struct {
	...
	in          io.Reader
	out         io.Writer
	home        string // ~/.vectoradb on this computer
	interactive bool
}

// Update runs `vdb update`: it installs the newest release — the engine where it
// runs, restarted servers, Blackbox upgrades on running branches, and `vdb` on
...
// returns a function that prints the one-line notice if one was found. `vdb
...
// build; VECTORADB_NO_UPDATE_CHECK=1 turns it off.
...
func (u *updater) run(ctx context.Context) error {
	...
	if err != nil || cur.IsDev() {
		return fmt.Errorf("this is a development build (%s) — `vdb update` updates installed releases; install one with the installer first", u.current)
	}
	t := u.hooks.target()
	u.printf("Checking for a newer VectoraDB release…\n")
	o, err := u.client.Resolve(ctx, cur, t, u.opts.Version)
	if err != nil {
		return fmt.Errorf("checking for updates: %w", err)
	}
	if o == nil {
		u.printf("VectoraDB %s is up to date.\n", u.current)
		return nil
	}
	u.printf("VectoraDB %s is available (you have %s).\n", o.Release.Tag, u.current)
	if o.Release.HTMLURL != "" {
		u.printf("  What's new: %s\n", o.Release.HTMLURL)
	}
	if u.opts.Check {
		u.printf("Run `vdb update` to install it.\n")
		return nil
	}
	if !u.opts.Yes {
		if !u.interactive {
			return fmt.Errorf("not a terminal — run `vdb update --yes` to update without the confirmation prompt")
		}
		...
		if rerr != nil && a == "" {
			...
			return fmt.Errorf("no answer — run `vdb update --yes` to update without the confirmation prompt")
		}
		if a != "y" && a != "yes" {
	...
	if err := u.eh.run(installed, "up"); err != nil {
		return fmt.Errorf("the new engine is installed but `vdb up` failed: %w\nFix the problem above, then run `vdb start`", err)
	}
	ledgerOK := u.eh.run(installed, "ledger", "upgrade", "--all") == nil
	if !ledgerOK {
		u.printf("  note: some branches weren't upgraded — run `vdb blackbox upgrade --all` later\n")
	}
	if err := u.eh.run(installed, "start"); err != nil {
		return fmt.Errorf("the new engine is installed but `vdb start` failed: %w\nFix the problem above, then run `vdb start`", err)
	}

	// 6. `vdb` on this computer.
	if u.hooks.replaceHost != nil {
		step("Updating vdb on this computer")
		if err := u.hooks.replaceHost(files, t); err != nil {
			return fmt.Errorf("the engine is on %s, but vdb on this computer wasn't replaced: %w\nReinstall with the installer to finish (VDB_VERSION=%s)", o.Release.Tag, err, o.Release.Tag)
		}
	}
	...
	if u.hooks.checkServers != nil {
		if err := u.hooks.checkServers(); err != nil {
			u.printf("\nnote: the control plane isn't answering yet (%v) — check `vdb status`\n", err)
		}
	}
	u.printf("\nDone — VectoraDB is now %s. Your data was not touched.\n", o.Release.Tag)
	if o.Release.HTMLURL != "" {
		u.printf("  What's new: %s\n", o.Release.HTMLURL)
	}
	u.printf("  To go back, reinstall %s with the installer (VDB_VERSION=v%s); the previous binaries are kept in ~/.vectoradb/updates/prev\n", u.current, u.current)
	return nil
}

// hostExecutable is the running vdb, with symlinks resolved.
...
// refreshEngineCache replaces the engine `vdb setup` would install from the
