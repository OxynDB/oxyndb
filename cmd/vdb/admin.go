...
// adminCmd handles `vdb admin grant|revoke <email> [--branch <name>]` and
// `vdb admin list [--branch <name>]`.
//
// Members of vdb_admin may override the destructive-DDL guardrail with
// SET vdb.allow_destructive=on; other users cannot. Roles live inside each
...
func adminCmd(args []string) {
	...
	case "grant", "revoke":
		...
		if !ok {
			must(fmt.Errorf("no such user: %s (create it with: vdb user create %s)", email, email))
		}
		done := map[string]string{"grant": "granted vdb_admin to", "revoke": "revoked vdb_admin from"}[sub]
...
func adminUsage() {
	fmt.Println("usage: vdb admin grant <email> [--branch <name>]\n" +
		"       vdb admin revoke <email> [--branch <name>]\n" +
		"       vdb admin list [--branch <name>]")
	os.Exit(2)
}
...
func adminScope(named string) ([]string, error) {
	...
	if len(running) == 0 || running[0] != "main" {
		return nil, fmt.Errorf("main is not running — start VectoraDB first (vdb start)")
	}
	return running, nil
}
