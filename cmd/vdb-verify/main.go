...
func main() {
	...
	export := flag.String("export", "", "check a `vdb ledger export` JSON-lines file instead of a live database")
	...
	flag.Usage = func() {
		fmt.Fprintln(os.Stderr, "usage: vdb-verify (--dsn <postgres-url> | --export <file.jsonl>) --anchors <dir> [--json]")
		flag.PrintDefaults()
	}
	flag.Parse()
	if *showVersion {
		fmt.Printf("vdb-verify %s\n", version.Version)
		return
	}
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "vdb-verify:", err)
		os.Exit(2)
	}
	...
	if len(anchors) == 0 {
		fmt.Fprintf(os.Stderr, "vdb-verify: warning: no anchor files in %s — only the hash chain can be checked\n", *anchorDir)
	}
...
// readExport parses a `vdb ledger export` file (one JSON ledger row per line).
...
// access to vdb.schema_ledger (and vdb.ledger_ext, if present) is all it needs —
// a connection through the VectoraDB gateway with an API key works.
func readDatabase(dsn string) ([]ledger.Row, error) {
	...
	var withExt bool
	if err := conn.QueryRow(ctx, "SELECT to_regclass('vdb.ledger_ext') IS NOT NULL").Scan(&withExt); err != nil {
