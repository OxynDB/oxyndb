...
Every ledger row is hash-chained inside the database (`vdb.schema_ledger`:
...
Anchors are only as safe as where they are kept. By default VectoraDB writes them
read-only (mode `0444`) to `~/.vectoradb/anchors/<branch>/`. Point
`VECTORADB_ANCHOR_DIR` at storage the database's operators cannot rewrite (a
write-once mount, a copy synced off the host) for stronger guarantees, and set
`VECTORADB_ANCHOR_IMMUTABLE=1` to also mark files immutable (`chattr +i`) where
...
| Field | Type | Meaning |
|---|---|---|
| `format` | string | Always `vectoradb-ledger-anchor/1`. Reject anything else. |
| `algorithm` | string | Always `sha256-merkle-v1` (defined below). |
| `branch` | string | Branch the ledger belongs to. |
| `checkpoint_id` | integer | Id of the checkpoint row in `vdb.ledger_checkpoints`. Informational. |
...
`(at AT TIME ZONE 'UTC')::text`). This is the same value as `vdb._ledger_hash`.
...
2. Leaf for each row: `SHA-256(0x00 ‖ row_hash ‖ "|" ‖ ext_hash)`, where
   ...
   capture hash from `vdb.ledger_ext.ext_hash` as ASCII hex (empty if the row has
   no capture row).
...
```bash
# against a live branch, through the VectoraDB gateway with an API key
vdb-verify --dsn 'postgresql://vectoradb:<api-key>@localhost:6432/main?sslmode=require' \
           --anchors ~/.vectoradb/anchors/main

# offline, against an export
vdb ledger export main > main-ledger.jsonl
vdb-verify --export main-ledger.jsonl --anchors ./anchors/main
```
