...
OxynDB's engine is Linux-only (ZFS + Docker + Postgres). On Windows it runs inside a dedicated
**WSL2** distro — the direct analog of the Lima VM used on macOS. The native `odb.exe` launcher
forwards every engine command into that distro, so day to day you just type `odb …`.
...
Linux distribution — OxynDB installs WSL if it's missing and brings its own dedicated distro.
...
2. downloads `odb.exe` into `%LOCALAPPDATA%\Programs\oxyndb` and adds it to your PATH — including
   the current window, so `odb` works immediately;
3. runs `odb setup`, which imports the dedicated `oxyndb` distro, installs Docker, downloads and
...
`%LOCALAPPDATA%\Programs\oxyndb\install.log`, which is named in any error.
...
To install without running setup, or without the admin prompt, set `ODB_NO_SETUP=1` or
`ODB_NO_ELEVATE=1` before running the command.

After that, use OxynDB exactly as on macOS/Linux — `odb branch create`, `odb import`, `odb status`,
...
The stock WSL2 kernel ships **no ZFS module**, and ZFS is what powers OxynDB's instant
copy-on-write branches. OxynDB does **not** solve that by replacing your kernel.
...
So `odb setup` writes `zfs.ko` into the **`oxyndb` distro's** upper layer and runs `depmod`. Your
...
The trade-off is that the modules are tied to one exact kernel version. `odb setup` reads `uname -r`
...
| `odb is not recognized` after install | The installer adds it to the current window's PATH as well as persisting it, so this should not happen. In a window opened *before* installing: `$env:Path += ";$env:LOCALAPPDATA\Programs\oxyndb"`. |
...
| `ZFS is not usable in the "oxyndb" distro` | The staged modules were built for a different kernel. Check `wsl -d oxyndb -- uname -r` against the bundle filename in `%LOCALAPPDATA%\Programs\oxyndb`. |
| `systemd did not finish booting` | `wsl --terminate oxyndb`, then re-run `odb setup`. |
| `the OxynDB ZFS pool device is not ready` | Deliberate stop: the pool could not be imported, and OxynDB will not run the engine in case it recreates the pool over your data. Run `wsl --terminate oxyndb` and retry; if it persists, see `journalctl -u oxyndb-zpool.service` inside the distro. |
| `pool I/O is currently suspended` | The pool lost its backing device. `wsl --terminate oxyndb` and re-run `odb setup`; the pool is re-imported at boot. |
...
```powershell
wsl --unregister oxyndb                       # remove the distro + its data
wsl --shutdown                                   # release its loop device
Remove-Item -Recurse "$env:LOCALAPPDATA\Programs\oxyndb"
Remove-Item -Recurse "$env:LOCALAPPDATA\oxyndb"
```

Nothing else to undo — OxynDB made no machine-wide WSL changes.
...
> a device pointing at the deleted pool image. `odb setup` detects this and stops
...
userland as `dist/oxyndb-zfs-<kernelrelease>.tar.gz`. The kernel image itself is not shipped.
